...
int alloc_and_load_placement_macros(t_direct_inf* directs, int num_directs, t_pl_macro ** chains){
	...
	/* This function allocates and loads the chains placement macros and returns the total
	 * number of chains. */
	...
	for (ichain = 0; ichain < num_chain; ichain++) {
		...
		chain[ichain].idirect = pl_macro_idirect[ichain];

		/* Load the values for each member of the chain */
		for (imember = 0; imember < chain[ichain].num_blocks; ) {
			chain[ichain].members[imember].x_offset = directs[chain[ichain].idirect].x_offset;
			chain[ichain].members[imember].y_offset = directs[chain[ichain].idirect].y_offset;
	...
	/* dont know the size of these
		1. get the size when I alloc and load
		2. write new subroutines to free these
