...
static void build_textarea(void) {
	...
	textarea = XCreateSimpleWindow(display, toplevel, 0,
			top_height - T_AREA_HEIGHT, display_width, T_AREA_HEIGHT - 4, 2,
...
static void build_default_menu(void) {
	...
	menu = XCreateSimpleWindow(display, toplevel, top_width - MWIDTH, 0,
			MWIDTH - 4, display_height, 2, colors[BLACK], colors[LIGHTGREY]);
	menu_attributes.event_mask = ExposureMask;
...
static void map_button(int bnum) {
	...
	button[bnum].win = XCreateSimpleWindow(display, menu, button[bnum].xleft,
			button[bnum].ytop, button[bnum].width, button[bnum].height, 0,
...
void destroy_button(char *button_text) {
	...
	bnum = -1;
	for (i = 4; i < num_buttons; i++) {
		if (button[i].istext == 1 && strcmp(button[i].text, button_text) == 0) {
			bnum = i;
			break;
		}
	}
...
void init_graphics(const char *window_name) {
	...
	toplevel = XCreateSimpleWindow(display, RootWindow(display, screen_num), x,
			y, top_width, top_height, border_width, colors[BLACK],
			colors[WHITE]);
	...
	XDrawString(display, win, gc_menus, xc - width / 2,
			yc
					+ (font_info[menu_font_size]->ascent
							- font_info[menu_font_size]->descent) / 2, text,
			len);
}
...
void drawline(float x1, float y1, float x2, float y2) {
	...
	if (disp_type == SCREEN) {
		...
		XDrawLine(display, toplevel, gc, xcoord(x1), ycoord(y1), xcoord(x2),
				ycoord(y2));
	} else {
...
void drawtext(float xc, float yc, const char *text, float boundx) {
	...
	if (disp_type == SCREEN) {
		XDrawString(display, toplevel, gc, xcoord(xc) - width / 2,
				ycoord(yc)
						+ (font_info[currentfontsize]->ascent
								- font_info[currentfontsize]->descent) / 2,
				text, len);
	} else {
...
void draw_message(void) {
	...
	if (disp_type == SCREEN) {
		...
		XDrawString(display, textarea, gc_menus,
				...
				(T_AREA_HEIGHT - 4) / 2
						+ (font_info[menu_font_size]->ascent
								- font_info[menu_font_size]->descent) / 2,
				message, len);
	}
...
int init_postscript(char *fname) {
	...
	fprintf(ps, "%%%%BoundingBox: %d %d %d %d\n", (int) ps_left,
			(int) (ps_bot - 15.), (int) ps_right, (int) ps_top);
