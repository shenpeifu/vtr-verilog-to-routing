    ...
    for(i = 0; i <= (nx + 1); i++)
	{
	    for(j = 0; j <= (ny + 1); j++)
		{
		    ...
		    if(0 == ofs)
			{
			     ...
			     * are disjoint so they can share the list. */
/* JR-041212			    tmp.nelem = type->num_pins; */
			    tmp.nelem = type->num_pins + type->num_interconnects;
			    tmp.list = NULL;
			    if(tmp.nelem > 0)
				{
				    tmp.list =
		  ...
		  t_ivec *** rr_node_indices)
{
	...
	case OPIN:
/* JR-041212	    assert(ptc < type->num_pins); */
	    assert(ptc < type->num_pins + type->num_interconnects);
