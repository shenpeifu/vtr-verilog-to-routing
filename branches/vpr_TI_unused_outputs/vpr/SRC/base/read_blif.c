...
/* JR-031412 */
static boolean lut_has_unused_truth_table( char* buffer );
...
/* JR-031412 			add_vpack_net(saved_names[j], RECEIVER, num_logical_blocks - 1, 0, j, FALSE, doall); */
			add_vpack_net(saved_names[j], RECEIVER, num_logical_blocks - 1, 0, j, FALSE, doall, FALSE);
		free_matrix(saved_names, 0, logic_model->inputs->size, 0, sizeof(char));
	    return;
	}
...
/* JR-031412 		logical_block[num_logical_blocks - 1].input_nets[0][i] = add_vpack_net(saved_names[i], RECEIVER, num_logical_blocks - 1, 0, i, FALSE, doall); */
		logical_block[num_logical_blocks - 1].input_nets[0][i] = add_vpack_net(saved_names[i], RECEIVER, num_logical_blocks - 1, 0, i, FALSE, doall, FALSE);
/* JR-031412     logical_block[num_logical_blocks - 1].output_nets[0][0] = add_vpack_net(saved_names[output_net_index], DRIVER, num_logical_blocks - 1, 0, 0, FALSE, doall); */
    logical_block[num_logical_blocks - 1].output_nets[0][0] = add_vpack_net(saved_names[output_net_index], DRIVER, num_logical_blocks - 1, 0, 0, FALSE, doall, FALSE);
...
/* JR-031412 	    add_vpack_net(saved_names[0], RECEIVER, num_logical_blocks - 1, 0, 0, FALSE, doall);	/* D */
...
/*  		    add_vpack_net(saved_names[3], RECEIVER, num_logical_blocks - 1, 0, 0, TRUE, doall);	/* Clock */
	    add_vpack_net(saved_names[0], RECEIVER, num_logical_blocks - 1, 0, 0, FALSE, doall, FALSE);	/* D */
	    add_vpack_net(saved_names[1], DRIVER, num_logical_blocks - 1, 0, 0, FALSE, doall, FALSE);	/* Q */
	    add_vpack_net(saved_names[3], RECEIVER, num_logical_blocks - 1, 0, 0, TRUE, doall, FALSE);	/* Clock */
	    return;
	}
...
/* JR-031412	logical_block[num_logical_blocks - 1].output_nets[0][0] = add_vpack_net(saved_names[1], DRIVER, num_logical_blocks - 1, 0, 0, FALSE, doall);	/* Q */
...
/* 		logical_block[num_logical_blocks - 1].clock_net = add_vpack_net(saved_names[3], RECEIVER, num_logical_blocks - 1, 0, 0, TRUE, doall);	/* Clock */
    logical_block[num_logical_blocks - 1].output_nets[0][0] = add_vpack_net(saved_names[1], DRIVER, num_logical_blocks - 1, 0, 0, FALSE, doall, FALSE);	/* Q */
    logical_block[num_logical_blocks - 1].input_nets[0][0] = add_vpack_net(saved_names[0], RECEIVER, num_logical_blocks - 1, 0, 0, FALSE, doall, FALSE);	/* D */
    logical_block[num_logical_blocks - 1].clock_net = add_vpack_net(saved_names[3], RECEIVER, num_logical_blocks - 1, 0, 0, TRUE, doall, FALSE);	/* Clock */
...
/* JR-031412 				add_vpack_net(circuit_signal_name[subckt_index_signals], RECEIVER, num_logical_blocks - 1, 0, 0, FALSE, doall); */
				add_vpack_net(circuit_signal_name[subckt_index_signals], RECEIVER, num_logical_blocks - 1, 0, 0, FALSE, doall, FALSE);
			}
...
/* JR-031412 							add_vpack_net(circuit_signal_name[i], RECEIVER, num_logical_blocks - 1, port->index, my_atoi(pin_number), TRUE, doall); */
							add_vpack_net(circuit_signal_name[i], RECEIVER, num_logical_blocks - 1, port->index, my_atoi(pin_number), TRUE, doall, FALSE);
					} else {
						logical_block[num_logical_blocks-1].input_nets[port->index][my_atoi(pin_number)] = 
/* JR-031412 							add_vpack_net(circuit_signal_name[i], RECEIVER, num_logical_blocks - 1, port->index, my_atoi(pin_number), FALSE, doall); */
							add_vpack_net(circuit_signal_name[i], RECEIVER, num_logical_blocks - 1, port->index, my_atoi(pin_number), FALSE, doall, FALSE);
						input_net_count++;
					}
				}
				port = port->next;
...
/* JR-031412 						add_vpack_net(circuit_signal_name[i], DRIVER, num_logical_blocks - 1, port->index, my_atoi(pin_number), FALSE, doall); */
						add_vpack_net(circuit_signal_name[i], DRIVER, num_logical_blocks - 1, port->index, my_atoi(pin_number), FALSE, doall, FALSE);
...
/*  			        error++; */
			if( !ignore_unused_outputs) {
				...
				printf(WARNTAG "vpack_net %s has" " %d signals driving it.\n", vpack_net[i].name, num_driver[i]);
			}
		}
...
/* JR-031412 */
	    if(vpack_net[i].num_sinks == -1)
		{
			...
			if(ignore_unused_outputs) {
				...
				vpack_net[i].node_block_port[0] = OPEN;
				vpack_net[i].node_block_pin[0] = OPEN;
				logical_block[iblk].input_nets[iport][ipin] = OPEN;
				logical_block_input_count[iblk]--;					
			}			    
		}
...
/* JR-031412 */
/*===========================================================================//
// Function       : lut_has_unused_truth_table
// Description    : Returns true if current LUT (ie. ".names) has an 'unused'
//                  truth table (ie. a '0' or constant ground generator).
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 03/14/12 jeffr : Original
//===========================================================================*/
static boolean lut_has_unused_truth_table( char* buffer )
{
   boolean unused_truth_table = FALSE;

   char* ptr;

   #define TOKENS " \t\n"
   if( my_fpeek( buffer, BUFSIZE, TOKENS, blif ))
   {
      ptr = my_strtok( buffer, TOKENS, blif, buffer );
      if( ptr && 
          strlen( ptr ) == 1 && 
          ptr[0] == '0' )
      {
         ...
         if( !ptr ) 
         {
            unused_truth_table = TRUE;
         }
      }
   }
   return( unused_truth_table );
}
