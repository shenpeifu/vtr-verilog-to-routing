...
/*===========================================================================//
// Function       : my_fpeek
// Description    : Peeks and returns next input line, if any.
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 03/14/12 jeffr : Original
//===========================================================================*/
char* my_fpeek( char* buf, int max_size, char* tokens, FILE* fp )
{
   int pos;
   char* val;
   char* ptr;
   ...
   val = my_fgets( buf, max_size, fp );
   if( val )
   {
      ptr = my_strtok(buf, tokens, fp, buf);
      if( !ptr )
      {
         val = my_fpeek( buf, max_size, tokens, fp );
      }
   }
