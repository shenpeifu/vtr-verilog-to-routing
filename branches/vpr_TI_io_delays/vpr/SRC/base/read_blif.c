...
/* JR-030812 */
static void add_delay(int doall);
...
/*===========================================================================//
// Function       : add_delay
// Description    : Adds IO delay to an .input or .output logical block.
//                  Format is assumed to be: .io_delay <io_name> <io_delay>
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 03/08/12 jeffr : Original
//===========================================================================*/
static void add_delay( int doall )
{
   char* ptr;
   char buf[BUFSIZE];
   char io_name[BUFSIZE];
   char io_delay[BUFSIZE];
   ...
   if( !*io_name || !*io_delay )
   {
      fprintf( stderr, "Error: .io_delay does not have 2 parameters.\n" 
                       "Check the netlist, line %d.\n", linenum );
      exit( 1 );
   }
   ...
   if( doall )
   {
             ...
             logical_block[ i ].type != VPACK_OUTPAD ) 
	 {
         ...
         {
            if( !strcmp( logical_block[ i ].name, io_name ) == 0 )
               continue;
         } 
         else if( logical_block[ i ].type == VPACK_OUTPAD ) 
         {
            if( strlen( logical_block[ i ].name ) < 4 ||
                !strcmp( logical_block[ i ].name + 4, io_name ) == 0 )
               continue;
         } 

         logical_block[ i ].delay = atof( io_delay );
         break;
      }
   }
}
