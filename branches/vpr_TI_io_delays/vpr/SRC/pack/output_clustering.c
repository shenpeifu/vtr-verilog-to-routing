...
/* JR-030812	} else { */
/* JR-030812 		fprintf(fpout, "<block name=\"%s\" instance=\"%s[%d]\" mode=\"%s\">\n", pb->name, pb_type->name, pb_index, mode->name); */
	} else if(delay > FLT_EPSILON) {
		fprintf(fpout, "<block name=\"%s\" instance=\"%s[%d]\" mode=\"%s\" delay=\"%0.3f\">\n", pb->name, pb_type->name, pb_index, mode->name, delay);
	} else {
...
/* JR-030812     if(skip_clustering == FALSE) */
/* JR-030812 		print_clusters(clb, num_clusters, fpout); */
	if(skip_clustering == FALSE) {
		for(iblk = 0; iblk < num_logical_blocks; iblk++) {
			if(!logical_block)
				continue;

			logical_block[iblk].pb->delay = logical_block[iblk].delay;
			if( logical_block[iblk].pb->parent_pb )
				logical_block[iblk].pb->parent_pb->delay = logical_block[iblk].delay;
		}
