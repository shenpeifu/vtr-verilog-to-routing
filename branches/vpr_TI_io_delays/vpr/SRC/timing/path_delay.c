...
static void alloc_and_load_tnodes(t_timing_inf timing_inf) {
	...
	int count;
/* JR-030812	int iblock, irr_node; */
	int iblock, jblock;
	int irr_node, jrr_node;
	...
	for(i = 0; i < num_tnodes; i++) {
		...
		*/
		...
		switch (tnode[i].type) {
			...
			case OUTPAD_IPIN:
/* JR-030812 */
				/* Load edge to OUTPAD_SINK based on optional user-defined IO delay */
				if(block[iblock].pb->delay > FLT_EPSILON)
					tnode[i].out_edges[0].Tdel = block[iblock].pb->delay;
				else
					tnode[i].out_edges[0].Tdel = timing_inf.T_ipin_sink_io;
				break;

			case INPAD_SOURCE:
/* JR-030812 */
				/* Load edge to INPAD_OPIN based on optional user-defined IO delay */
				if(block[iblock].pb->delay > FLT_EPSILON)
					tnode[i].out_edges[0].Tdel = block[iblock].pb->delay;
				else
					tnode[i].out_edges[0].Tdel = timing_inf.T_source_opin_io;
				break;
