...
/* JR-021412 */
static boolean cluster_unique_clock_feasible( int iblock, t_pb* pb, t_pb* parent_pb );
static boolean cluster_sibling_clock_feasible( int iblock, t_pb* pb );
static boolean cluster_netpins_clock_feasible( int iblock, int inet, t_pb* pb );
...
/* JR-021412 */
	if( !cluster_unique_clock_feasible( iblk, cur_pb, 0 ) || !cluster_sibling_clock_feasible( iblk, cur_pb )) {
		return FALSE;
        }
...
/* JR-021412 */
/*===========================================================================//
// Function       : cluster_unique_clock_feasible
// Description    : Searches all primitive nodes associated with the given
//                  physical block to determine if the given clock net is 
//                  'feasible'. This implementation currently ASSUMES only 
//                  one clock is defined per logic block. Therefore, any 
//                  primitive nodes found with a different clock net will 
//                  result in a feasibility failure.
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 02/14/12 jeffr : Original
//===========================================================================*/
static boolean cluster_unique_clock_feasible( int iblock, t_pb* pb, t_pb* parent_pb )
{
   ...
   if(( pb ) &&
      ( parent_pb ) &&
      ( logical_block[iblock].clock_net != OPEN ))
   {
      /* Handle case where we apply current physical block, then travese children */
      if( pb->logical_block != OPEN )
      {
         /* We have arrived at a primitive node, ready to test clock net */
         int jblock = pb->logical_block;

         if(( logical_block[jblock].clock_net != OPEN ) &&
      ...
      else if( pb->child_pbs )
      {
         ...
         for( i = 0; i < pb_type->modes[mode].num_pb_type_children; ++i ) 
         {
	       ...
	       if( !child_pb )
               ...
               is_feasible = cluster_unique_clock_feasible( iblock, child_pb, parent_pb );
               if( !is_feasible )
                  break;
            }
            if( !is_feasible )
      ...
      ( logical_block[iblock].clock_net != OPEN ))
   {
      ...
      is_feasible = cluster_unique_clock_feasible( iblock, parent_pb, parent_pb );
   }
   return( is_feasible );
}
...
/* JR-021412 */
/*===========================================================================//
// Function       : cluster_sibling_clock_feasible
// Description    : Searches all sibling logical blocks with respect to the 
//                  given logical block to determine if each sibling's clock
   ...
   if( logical_block[iblock].type == VPACK_LATCH )
   {
      ...
      while( input_port && is_feasible )
      {
         ...
         for( ipin = 0; ipin < input_port->size; ++ipin ) 
         {
            ...
            is_feasible = cluster_netpins_clock_feasible( iblock, input_net, pb );
            if( !is_feasible )
               break;
         }
   ...
   if( logical_block[iblock].type == VPACK_COMB )
   {
      ...
      while( output_port && is_feasible )
      {
         ...
         for( ipin = 0; ipin < output_port->size; ++ipin ) 
         {
            ...
            is_feasible = cluster_netpins_clock_feasible( iblock, output_net, pb );
            if( !is_feasible )
               break;
         }
...
/* JR-021412 */
/*===========================================================================//
// Function       : cluster_netpins_clock_feasible
// Description    : Searches all logical blocks assoicated with a given net 
//                  to determine if the given clock net is 'feasible'.
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 02/14/12 jeffr : Original
...
static boolean cluster_netpins_clock_feasible( int iblock, int inet, t_pb* pb )
{
      ...
      ( inet != OPEN ))
   {
	 ...
	 if( iblock == jblock )
         ...
         if( !cluster_unique_clock_feasible( jblock, pb, 0 ))
         {
            is_feasible = FALSE;
            break;
         }
