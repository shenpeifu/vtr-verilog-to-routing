...
foreach my $type (@transistor_types) {
	...
	( $C_gate_cmos, $C_drain_cmos, $C_gate_pass, $C_drain_pass, $C_source_pass ) = get_capacitances( $type, $long_size[0], $long_size[1] );
	...
	foreach my $size_ref (@sizes) {
		my @size = @$size_ref;
		( $C_gate_cmos, $C_drain_cmos, $C_gate_pass, $C_drain_pass, $C_source_pass ) = get_capacitances( $type, $size[0], $size[1] );
...
sub print_buffer_sc {
	...
	for (my $S = 1.0; $S < $max_buffer_size; $S = $S * $buffer_interval) {
		my $N = calc_buffer_num_stages($S);
		my $g = calc_buffer_stage_effort($N, $S);
		
		for (my $i = 2; $i < $max_mux_size; $i++) 
		{
			my $sc = get_buffer_sc($N, $S, 1, $i, $pn_ratio);
			print kfjdsa
		}
		get_buffer_sc($N, $S, 0, 0, $pn_ratio);
		print dskjfdsaf
	}
}
...
sub get_buffer_sc {
	...
	if($level_restorer) {
		$s = $s . "Xmux0 in Vdd out0 0 nfet size='1'\n";
		for (my $i = 1; $i < $mux_in_size; $i = $i + 1) {
			$s = $s . "Xmux" . $i . " 0 0 out0 0 nfet size='1'\n";
		}
		$s = $s . "X0 out0 out1 VupL0 VdownH0 levr\n";
	} else {
		$s = $s . "XinA in inA Vdd 0 inv nsize='1' psize='" . $pn_ratio . "'\n";
		$s = $s . "XinB inA out0 Vdd 0 inv nsize='1' psize='" . $pn_ratio . "'\n";
		$s = $s . "X0 out0 out1 VupL0 VdownH0 inv nsize='1' psize='" . $pn_ratio . "'\n";
	}
	
	my $stage_size = $stage_gain;
	for (my $i = 1; $i < $stages; $i = $i + 1) {
		
		$s = $s . "X" . $i . " out" . $i . " out" . ($i+1) . " VupL" . $i . " VdownH" . $i . " inv nsize='" . $stage_size . "' psize='" . ($stage_size * $pn_ratio) . "'\n";
		$stage_size = $stage_size * $stage_gain;
	}
	...
	for (my $i = 0; $i < $stages; $i = $i + 1) {
		if ($up) {
			$s = $s . ".measure rs" . $i . " when V(out" . $i . ")='0.05*Vol' CROSS=1\n";
			$s = $s . ".measure re" . $i . " when V(out" . $i . ")='0.95*Vol' CROSS=1\n";
			$s = $s . ".measure fs" . $i . " when V(out" . $i . ")='0.95*Vol' CROSS=2\n";
			$s = $s . ".measure fe" . $i . " when V(out" . $i . ")='0.05*Vol' CROSS=2\n";
		} else {
			$s = $s . ".measure fs" . $i . " when V(out" . $i . ")='0.95*Vol' CROSS=1\n";
			$s = $s . ".measure fe" . $i . " when V(out" . $i . ")='0.05*Vol' CROSS=1\n";
			$s = $s . ".measure rs" . $i . " when V(out" . $i . ")='0.05*Vol' CROSS=2\n";
			$s = $s . ".measure re" . $i . " when V(out" . $i . ")='0.95*Vol' CROSS=2\n";
		}
		$up = ! $up;
		
		$s = $s . ".measure tran ITr". $i . " integ I(Vdown" . $i . ") FROM 'rs" . $i . "' TO 're" . $i . "'\n";
		$s = $s . ".measure tran ISCr". $i . " integ I(Vup" . $i . ") FROM 'rs" . $i . "' TO 're" . $i . "'\n";
		$s = $s . ".measure tran ITf". $i . " integ I(Vup" . $i . ") FROM 'fs" . $i . "' TO 'fe" . $i . "'\n";
		$s = $s . ".measure tran ISCf". $i . " integ I(Vdown" . $i . ") FROM 'fs" . $i . "' TO 'fe" . $i . "'\n";
	...
	$s = $s . ".measure tran fSC Param=('(" . $SCp . ")/(" . $T . $SCm . ")')\n";
...
sub spice_header {
	...
	$s = $s . ".param simt = " . $simt . "\n";
	$s = $s . ".param rise = 'simt/1000'\n";
	$s = $s . ".param fall = 'simt/1000'\n";
...
sub get_capacitances {
	...
	$s = $s . spice_end();
	my @results = spice_run( $s, [ "c_gate_cmos", "c_drain_cmos", "c_gate_pass", "c_drain_pass", "c_source_pass" ] );
