 ...
 * check that this makes sense, to verify the rr graph.                 */
	...
	case OPIN:
/* JR-041212	    assert(to_type == CHANX || to_type == CHANY); */
	    assert(to_type == CHANX || to_type == CHANY || to_type == IPIN);
...
/* JR-041212 */
	else if(chan_type == IPIN)
	{
	    if(chan_xlow == pin_xlow)
		{
		    if(chan_ylow == pin_ylow - 1 || chan_ylow == pin_ylow + 1)
			num_adj++;
		}
	    if(chan_ylow == pin_ylow)
		{
		    if(chan_xlow == pin_xlow - 1 || chan_xlow == pin_xlow + 1)
			num_adj++;
		}
	}
    return (num_adj);
