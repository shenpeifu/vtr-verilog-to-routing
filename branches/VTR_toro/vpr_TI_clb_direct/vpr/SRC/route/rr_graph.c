...
/* JR-041212 */
static void update_rr_graph_connect_direct_pin_edges( struct s_grid_tile** grid,
                                                   int grid_dx,
                                                   int grid_dy,
                                                   t_rr_node* rr_nodes,
                                                   t_ivec*** rr_nodes_indices,
                                                   int rr_nodes_len,
                                                   int num_interconnect_pins,
                                                   int num_nodes_per_channel,
                                                   int opin_to_ipin_switch );

static void add_rr_graph_connect_direct_pin_edges( struct s_grid_tile** grid,
                                                   int grid_dx,
                                                   int grid_dy,
                                                   t_rr_node* rr_nodes,
                                                   t_ivec*** rr_nodes_indices,
                                                   ...
                                                   int num_interconnect_pins,
                                                   int opin_to_ipin_switch );
static void add_rr_graph_connect_clb_opin_edge( struct s_grid_tile** grid,
                                                int grid_dx,
                                                int grid_dy,
                                                t_rr_node* rr_nodes,
                                                t_ivec*** rr_nodes_indices,
...
static void delete_rr_graph_connect_clb_pin_edges( struct s_grid_tile** grid,
                                                   int grid_dx,
                                                   int grid_dy,
                                                   t_rr_node* rr_nodes,
                                                   t_ivec*** rr_nodes_indices,
                                                   int num_interconnect_pins,
                                                   int num_nodes_per_channel );
static void delete_rr_graph_connect_clb_ipin_edges( t_rr_node* rr_nodes,
                                                    t_ivec*** rr_nodes_indices,
                                                    int to_inode,
                                                    int num_nodes_per_channel );

static void set_rr_graph_nodes_pin_side( t_rr_node* rr_nodes, 
                                         int rr_nodes_len );

static void append_rr_graph_node_edge( t_rr_node* rr_nodes,
                                       int rr_inode,
                                       int add_inode,
                                       int add_switch );
static void remove_rr_graph_node_edge( t_rr_node* rr_nodes,
                                       int rr_inode,
                                       int delete_inode );

static int find_rr_graph_grid_clb_pin( struct s_grid_tile** grid,
                                       int grid_x,
                                       int grid_y,
                                       t_rr_node* rr_nodes,
                                       t_ivec*** rr_nodes_indices,
...
static boolean is_rr_graph_connect_direct_clb( struct s_grid_tile** grid,
                                               int grid_x,
                                               int grid_y,
                                               int num_interconnect_pins );
static boolean is_rr_graph_connected_to_clb( int this_inode,
                                             t_rr_node* rr_nodes,
                                             t_ivec*** rr_nodes_indices,
                                             int rr_nodes_len,
                                             int grid_x,
					     int grid_y );
static boolean is_rr_graph_connected_to_clb_sink( int this_inode,
                                                  t_rr_node* rr_nodes,
                                                  t_ivec*** rr_nodes_indices,
                                                  int grid_x,
					          int grid_y );
...
/* JR-041212 */
                            num_interconnect_pins, opin_to_ipin_switch, 
...
/* JR-041212 		   global_route_switch, delayless_switch, */
		   global_route_switch, delayless_switch, num_interconnect_pins,
...
/* JR-041212 */
    /* Update rr_graph's input/output pins based on optional direct 'interconnect' */
    update_rr_graph_connect_direct_pin_edges( grid, nx, ny,
                                              rr_node, rr_node_indices,
                                              num_rr_nodes,
                                              num_interconnect_pins,
                                              nodes_per_chan,
                                              opin_to_ipin_switch );
}
...
/* JR-041212    num_pins = type->num_pins; */
    num_pins = type->num_pins + type->num_interconnects;
...
/* JR-041212 */
    fprintf(fp, "Side: %d  Fan-in: %d\n", rr_node[inode].side,
	    rr_node[inode].fan_in); 
...
/* JR-041212 */
/*===========================================================================//
// Function       : update_rr_graph_connect_direct_pin_edges
// Description    : Updates the rr_graph based on CLB-to-CLB direct connect 
//                  requirements, if any.  This involves replacing zero or 
//                  more existing CLB input/output pin interconnects to 
//                  neighboring route channels with direct interconnects to 
//                  neighboring CLBs.
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 03/30/12 jeffr : Original
//===========================================================================*/
static void update_rr_graph_connect_direct_pin_edges(
      ...
      int                  opin_to_ipin_switch )
{
   if( num_interconnect_pins )
   {
      /* Set all rr_graph pin node 'side' fields on all CLBs w.r.t. route channels */
      set_rr_graph_nodes_pin_side( rr_nodes, rr_nodes_len );

      /* Add direct CLB-to-CLB for first 'ith' pins on all sides of all CLBs */
      add_rr_graph_connect_direct_pin_edges( grid, grid_dx, grid_dy,
                                             rr_nodes, rr_nodes_indices, rr_nodes_len,
                                             num_interconnect_pins,
                                             opin_to_ipin_switch );
   }
}
...
/* JR-041212 */
/*===========================================================================//
// Function       : add_rr_graph_connect_direct_pin_edges
// Description    : Adds rr_graph direct connect edges from output pins to 
//                  associated input pins in neighboring CLBs.  Addition is 
//                  limited to the first 'ith' pins found per each CLB side.
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 03/30/12 jeffr : Original
//===========================================================================*/
static void add_rr_graph_connect_direct_pin_edges(
      struct s_grid_tile** grid,
      int                  grid_dx,
      int                  grid_dy,
      t_rr_node*           rr_nodes,
...
{
   int grid_x, grid_y;
   int clb_side, ith_pin;

   /* Iterate for each CLB within the grid core */
   for( grid_x = 1; grid_x <= grid_dx; ++grid_x )
   {
      for( grid_y = 1; grid_y <= grid_dy; ++grid_y )
      {
         /* Ignore CLBs that are not canidates for interconnect direct */
         if( !is_rr_graph_connect_direct_clb( grid, grid_x, grid_y, 
                                              num_interconnect_pins ))
         {
            continue;
         }

         /* Iterate for the first 'n' output pins found on each CLB side */
         for( clb_side = 0; clb_side < 4; ++clb_side ) /* TOP RIGHT BOTTOM LEFT */
         {
            /* Ignore output pins on grid core periphery (only connect CLB-to-CLB) */
            switch( clb_side )
            {
            case LEFT:   if( grid_x == 1 ) continue;       break;
            case RIGHT:  if( grid_x == grid_dx ) continue; break;
            case BOTTOM: if( grid_y == 1 ) continue;       break;
            case TOP:    if( grid_y == grid_dy ) continue; break;
            }

            for( ith_pin = 1; ith_pin <= num_interconnect_pins; ++ith_pin )
            {
               /* Find next output pin (edge assumed to be OPEN after prior delete) */
               int from_inode = find_rr_graph_grid_clb_pin( grid, grid_x, grid_y, 
                                                            rr_nodes, rr_nodes_indices,
                                                            OPIN, clb_side, ith_pin );
               if( from_inode == OPEN )
                  break;

               /* Add an edge from given output pin to neighbor input pin */
               add_rr_graph_connect_clb_opin_edge( grid, grid_x, grid_y, 
                                                   rr_nodes, rr_nodes_indices, rr_nodes_len,
                                                   clb_side, ith_pin,
                                                   from_inode,
                                                   opin_to_ipin_switch );
...
/* JR-041212 */
/*===========================================================================//
// Function       : add_rr_graph_connect_clb_opin_edge
// Description    : Adds rr_graph CLB output pin edge from an output pin to 
//                  associated input pin in neighboring CLB.
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 03/30/12 jeffr : Original
//===========================================================================*/
static void add_rr_graph_connect_clb_opin_edge(
      struct s_grid_tile** grid,
      int                  grid_x,
      int                  grid_y,
      t_rr_node*           rr_nodes,
      ...
      int                  opin_to_ipin_switch )
{
   ...
   int neighbor_x = grid_x;
   int neighor_y = grid_y;
   int neighbor_side = clb_side;
   ...
   case TOP:
      neighbor_x = grid_x;
      neighor_y = grid_y + 1;
      neighbor_side = BOTTOM;
      break;
   }

   /* Add direct connect if OPIN is connected to neighbor CLB's SINK */
   if( is_rr_graph_connected_to_clb( from_inode,
                                     rr_nodes, rr_nodes_indices, rr_nodes_len,
                                     neighbor_x, neighor_y ))
   {
      /* Connect output pin directly to neighboring CLBs asso. input pin */
      to_inode = find_rr_graph_grid_clb_pin( grid, neighbor_x, neighor_y, 
                                             rr_nodes, rr_nodes_indices,
                                             IPIN, neighbor_side, ith_pin );
      if( to_inode != OPEN )
      {
         /* Append a new edge from the current OPIN to neighbor's IPIN */
         /* (ie. re-alloc, copy, and extend existing edges and switches) */
         append_rr_graph_node_edge( rr_nodes, from_inode, to_inode,
                                    opin_to_ipin_switch );
         ++rr_nodes[to_inode].fan_in;
      }
   }
}
...
/* JR-041212 */
/*===========================================================================//
// Function       : delete_rr_graph_connect_clb_pin_edges
// Description    : Deletes rr_graph CLB input pin edges for all sides of all
//                  CLBs.  Deletion is limited to the first 'ith' input pins 
//                  found per each CLB side. This function deletes input pins 
//                  from each side based on the given interconnect direct 
//                  count.  However, this function does not touch output pins 
//                  since it assumes that output pins used for interconnect 
//                  have a connection to both the route channel (ie. nearest 
//                  two switch boxes) and to the nearest neighbor CLB.
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history 
// 03/30/12 jeffr : Original
//===========================================================================*/
static void delete_rr_graph_connect_clb_pin_edges(
      struct s_grid_tile** grid,
      int                  grid_dx,
      int                  grid_dy,
      t_rr_node*           rr_nodes,
      ...
      int                  num_nodes_per_channel )
{
   ...
   for( grid_x = 1; grid_x <= grid_dx; ++grid_x )
   {
      for( grid_y = 1; grid_y <= grid_dy; ++grid_y )
      {
         ...
         /* Iterate for the first 'n' input pins found on each CLB side */
         for( clb_side = 0; clb_side < 4; ++clb_side ) /* TOP RIGHT BOTTOM LEFT */
         {
            for( ith_pin = 1; ith_pin <= num_interconnect_pins; ++ith_pin )
            {
               int to_inode = find_rr_graph_grid_clb_pin( grid, grid_x, grid_y, 
                                                          rr_nodes, rr_nodes_indices,
                                                          IPIN, clb_side, ith_pin );
               if( to_inode == OPEN )
                  break;

               /* Delete all edges from route channel tracks to given input pin */
               delete_rr_graph_connect_clb_ipin_edges( rr_nodes, rr_nodes_indices,
                                                       to_inode, 
...
/* JR-041212 */
/*===========================================================================//
// Function       : delete_rr_graph_connect_clb_ipin_edges
// Description    : Deletes rr_graph CLB input pin edges based on the given 
//                  graph node index.  In this rr_graph DAG, the edges are
//                  assumed from routing channels to input pins.  Finding the 
//                  appropriate edge requires iterating on the neighboring 
//                  routing channel for edges connected to the given graph 
//                  node index.
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 03/30/12 jeffr : Original
//===========================================================================*/
static void delete_rr_graph_connect_clb_ipin_edges(
      t_rr_node* rr_nodes,
      t_ivec***  rr_nodes_indices,
      int        to_inode,
      int        num_nodes_per_channel )
{
   if( rr_nodes[to_inode].type == IPIN )
   {
      int x = rr_nodes[to_inode].xlow;
      int y = rr_nodes[to_inode].ylow;
 
      int itrack, i;
      for( itrack = 0; itrack < num_nodes_per_channel; ++itrack )
      {
         int from_inode = OPEN;

         switch( rr_nodes[to_inode].side )
         {
         case LEFT:
            from_inode = get_rr_node_index( x - 1, y, CHANY, itrack, rr_nodes_indices );
            break;
         case RIGHT:
            from_inode = get_rr_node_index( x, y, CHANY, itrack, rr_nodes_indices );
            break;
         case BOTTOM:
            from_inode = get_rr_node_index( x, y - 1, CHANX, itrack, rr_nodes_indices );
            break;
         case TOP:
            from_inode = get_rr_node_index( x, y, CHANX, itrack, rr_nodes_indices );
            break;
         default:
            break;
         }
         ...
         for( i = 0; i < rr_nodes[from_inode].num_edges; ++i )
         {
            if( rr_nodes[from_inode].edges[i] == to_inode )
            {
               /* Remove an existing edge from neighbor's CHANX/Y to current IPIN */
               /* (ie. re-alloc, discard, and copy existing edges and switches) */
   	       remove_rr_graph_node_edge( rr_nodes, from_inode, to_inode );
               --rr_nodes[to_inode].fan_in;
	       break;
            }
         }
...
/* JR-041212 */
/*===========================================================================//
// Function       : set_rr_graph_nodes_pin_side
// Description    : Sets all rr_graph pin node 'side' fields based on the CLB 
//                  side with respect to the pin's connected routing channel.
//                  By default, all other rr_graph node 'side' fields are 
//                  initialized to OPEN.
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 03/30/12 jeffr : Original
//===========================================================================*/
static void set_rr_graph_nodes_pin_side(
      t_rr_node* rr_nodes,
      int        rr_nodes_len )
{
   int i, j;
   ...
   /* Iterate all graph nodes to initialize a default 'side' (ie. OPEN) */
   for( i = 0; i < rr_nodes_len; ++i )
   {
      rr_nodes[i].side = OPEN;
   }

   /* Iterate all graph nodes again to set connected input/output pin 'side' */
   for( i = 0; i < rr_nodes_len; ++i )
   {
      int from_inode = OPEN;
      int to_inode = OPEN;

      switch( rr_nodes[i].type )
      {
      case OPIN:

         /* Update 'from' output pin's side based on 'to' channel's position */
         from_inode = i;
         for( j = 0; j < rr_nodes[from_inode].num_edges; ++j )
         {
            to_inode = rr_nodes[from_inode].edges[j];

            if( rr_nodes[to_inode].type == CHANX )
            {
               if( rr_nodes[to_inode].ylow < rr_nodes[from_inode].ylow )
                  rr_nodes[from_inode].side = BOTTOM;
               else
                  rr_nodes[from_inode].side = TOP;
            }
            if( rr_nodes[to_inode].type == CHANY )
            {
               if( rr_nodes[to_inode].xlow < rr_nodes[from_inode].xlow )
                  rr_nodes[from_inode].side = LEFT;
               else
                  rr_nodes[from_inode].side = RIGHT;
            }
         }
         break;
      ...
      case CHANX:
         ...
         /* Update 'to' input pin's side based on 'from' channel's position */
         to_inode = i;
         for( j = 0; j < rr_nodes[to_inode].num_edges; ++j )
         {
            from_inode = rr_nodes[to_inode].edges[j];

            if( rr_nodes[from_inode].type == IPIN )
            {
               if( rr_nodes[to_inode].ylow < rr_nodes[from_inode].ylow )
                  rr_nodes[from_inode].side = BOTTOM;
               else
   ...
   /* Iterate all graph nodes again to set remaining input pin 'side' */
   side = -1;
   for( i = 0; i < rr_nodes_len; ++i )
   {
      switch( rr_nodes[i].type )
	 ...
	 /* Update input pin's side based on sequential rotation assignment */
         if( rr_nodes[i].side == OPEN )
         {
            rr_nodes[i].side = ++side % 4;
         }
         break;
...
/* JR-041212 */
/*===========================================================================//
// Function       : append_rr_graph_node_edge
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 03/30/12 jeffr : Original
//===========================================================================*/
static void append_rr_graph_node_edge(
      t_rr_node* rr_nodes,
      int        rr_inode,
      int        add_inode,
      int        add_switch )
{
   ...
   int num_edges = rr_nodes[rr_inode].num_edges + 1;
   int* edges = rr_nodes[rr_inode].edges;
   short* switches = rr_nodes[rr_inode].switches;

   rr_nodes[rr_inode].edges = my_malloc( num_edges * sizeof( int ));
   rr_nodes[rr_inode].switches = my_malloc( num_edges * sizeof( short ));

   for( i = 0; i < rr_nodes[rr_inode].num_edges; ++i )
   {
      rr_nodes[rr_inode].edges[i] = edges[i];
      rr_nodes[rr_inode].switches[i] = switches[i];
   }
   free( edges );
   free( switches );

   rr_nodes[rr_inode].num_edges = num_edges;
   rr_nodes[rr_inode].edges[num_edges - 1] = add_inode;
   rr_nodes[rr_inode].switches[num_edges - 1] = add_switch;
}


/* JR-041212 */
/*===========================================================================//
// Function       : remove_rr_graph_node_edge
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 03/30/12 jeffr : Original
//===========================================================================*/
static void remove_rr_graph_node_edge(
      t_rr_node* rr_nodes,
      int        rr_inode,
      int        delete_inode )
{
   if( rr_nodes[rr_inode].num_edges > 0 )
   {
      int i;
      ...
      for( i = 0; i < rr_nodes[rr_inode].num_edges; ++i )
      {
         if( rr_nodes[rr_inode].edges[i] == delete_inode )
            continue;

         ++num_edges;
      }

      rr_nodes[rr_inode].edges = NULL;
      rr_nodes[rr_inode].switches = NULL;
      ...
      if( num_edges > 0 )
      {
         ...
         for( i = 0; i < rr_nodes[rr_inode].num_edges; ++i )
         {
            if( edges[i] == delete_inode )
               continue;

            rr_nodes[rr_inode].edges[num_edges] = edges[i];
            rr_nodes[rr_inode].switches[num_edges] = switches[i];
            ++num_edges;
         }
      }
      rr_nodes[rr_inode].num_edges = num_edges;
...
/* JR-041212 */
/*===========================================================================//
// Function       : find_rr_graph_grid_clb_pin
// Description    : Finds and returns the 'ith' input/output pin, if any,
//                  based on the given CLB side.
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 03/30/12 jeffr : Original
//===========================================================================*/
static int find_rr_graph_grid_clb_pin(
      struct s_grid_tile** grid,
      int                  grid_x,
      int                  grid_y,
      t_rr_node*           rr_nodes,
      ...
      int                  ith_pin )
{
   ...
   /* Iterate for all pins associated with the given CLB */
   for( ipin = 0; ipin < type->num_pins + type->num_interconnects; ++ipin )
   {
      if( type->is_global_pin[ipin] )
         continue;
      ...
      /* Limit search based on the given input/output pin type */
      if((( type->class_inf[iclass].type == RECEIVER ) && ( rr_type == IPIN )) ||
         (( type->class_inf[iclass].type == DRIVER ) && ( rr_type == OPIN )))
      {
         inode = get_rr_node_index( grid_x, grid_y, rr_type, ipin, rr_nodes_indices );
         if( inode == OPEN )
            continue;

         /* And, limit search based on the given CLB side */ 
         if( rr_nodes[inode].side == clb_side )
         {
            /* And, limit search based on the given 'ith' pin found on side */ 
            --ith_pin;
            if( ith_pin <= 0 )
               break;
         }
...
/* JR-041212 */
/*===========================================================================//
// Function       : is_rr_graph_connect_direct_clb
// Description    : Returns TRUE is given CLB is a candidate for applying 
//                  internconnect direct processing.
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 03/30/12 jeffr : Original
//===========================================================================*/
static boolean is_rr_graph_connect_direct_clb(
      struct s_grid_tile** grid,
      int                  grid_x,
      int                  grid_y,
      int                  num_interconnect_pins )
{
   boolean is_connect_direct = FALSE;

   t_type_ptr type;
   type = grid[grid_x][grid_y].type;

   if(( type->pb_type->num_output_ports == 1 ) &&
      ( type->pb_type->num_input_ports == 1 ) &&
      ( type->pb_type->num_output_pins >= 4 * num_interconnect_pins ) &&
      ( type->pb_type->equivalent_input_pins ))
   {
      is_connect_direct = TRUE;
   }
   return( is_connect_direct );
}


/* JR-041212 */
/*===========================================================================//
// Function       : is_rr_graph_connected_to_clb
// Description    : Returns TRUE if any SINK nodes found within a given CLB.
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 03/30/12 jeffr : Original
//===========================================================================*/
static boolean is_rr_graph_connected_to_clb(
      int        this_inode,
      t_rr_node* rr_nodes,
      t_ivec***  rr_nodes_indices,
      int        rr_nodes_len,
      ...
      int        grid_y )
{
   ...
   for( i = 0; i < rr_nodes_len; ++i )
   {
      rr_nodes[i].is_visited = FALSE;
   }

   is_connected = is_rr_graph_connected_to_clb_sink( this_inode,
                                                     rr_nodes, rr_nodes_indices,
                                                     grid_x, grid_y );
   return( is_connected );
}
...
/* JR-041212 */
/*===========================================================================//
// Function       : is_rr_graph_connected_to_clb_sink
// Description    : Recursively iterates the rr_graph DAG from a given node 
//                  to all SINK nodes.  Returns TRUE if any SINK nodes are 
//                  found within the given CLB.
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 03/30/12 jeffr : Original
//===========================================================================*/
static boolean is_rr_graph_connected_to_clb_sink(
      int        this_inode,
      t_rr_node* rr_nodes,
      t_ivec***  rr_nodes_indices,
      int        grid_x,
      int        grid_y )
{
   ...
   /* Set flag indicating this node has been visited (to handle non-DAG paths) */
   rr_nodes[this_inode].is_visited = TRUE;

   switch( rr_nodes[this_inode].type )
   {
   case SOURCE:
   case IPIN:
   case OPIN:
   ...
   case CHANY:
      ...
      /* Iterate and traverse (recursively) for all node edges */
      for( i = 0; i < rr_nodes[this_inode].num_edges; ++i )
      {
	 int next_inode = rr_nodes[this_inode].edges[i];
	 if( next_inode == OPEN )
            continue;

 	 if( rr_nodes[next_inode].is_visited )
            continue;

         is_connected = is_rr_graph_connected_to_clb_sink( next_inode,
                                                           rr_nodes, rr_nodes_indices,
                                                           grid_x, grid_y );
         if( is_connected )
            break;
   ...
   case SINK:
      ...
      /* Test if SINK node is a member of the given CLB (based on CLB coordinates) */
      sink_indices = rr_nodes_indices[SINK][grid_x][grid_y];
      for( i = 0; i < sink_indices.nelem; ++i )
      {
         if( this_inode == sink_indices.list[i] )
         {
            is_connected = TRUE;
            break;
         }
