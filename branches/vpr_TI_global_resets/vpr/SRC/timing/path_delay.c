...
/* JR-022412 */
		logical_block[i].reset_net_tnode = NULL;
		if(logical_block[i].type == VPACK_INPAD) {
			...
			while(model_port) {
/* JR-022412				if(model_port->is_clock == FALSE) { 	*/
				if(!model_port->is_clock && !model_port->is_reset) {
...
/* JR-022412 */
				} else if(model_port->is_reset) {
					if(logical_block[i].reset_net != OPEN) {
						assert(logical_block[i].reset_net_tnode == NULL);
						logical_block[i].reset_net_tnode = &tnode[inode];
						tnode[inode].block = i;
						tnode[inode].model_pin = 0;
						tnode[inode].model_port = 0;
						tnode[inode].num_edges = 0;
...
/* JR-022412 						assert(logical_block[vpack_net[inet].node_block[j]].clock_net == inet); 					*/
/* JR-022412 						tnode[i].out_edges[j - 1].to_node = logical_block[vpack_net[inet].node_block[j]].clock_net_tnode->index; 	*/

						if(logical_block[vpack_net[inet].node_block[j]].clock_net == inet) {
							tnode[i].out_edges[j - 1].to_node = logical_block[vpack_net[inet].node_block[j]].clock_net_tnode->index;
						} else if(logical_block[vpack_net[inet].node_block[j]].reset_net == inet) {
							tnode[i].out_edges[j - 1].to_node = logical_block[vpack_net[inet].node_block[j]].reset_net_tnode->index;
						}
					} else {
...
/* JR-022412 */
		} else if (tnode[i].pb_graph_pin->type == PB_PIN_RESET) {
			tnode[i].type = FF_SINK;
			tnode[i].num_edges = 0;
			tnode[i].out_edges = NULL;				
		} else {
...
static void print_primitive_as_blif (FILE *fpout, int iblk) {
	...
	if(logical_block[iblk].type == VPACK_INPAD) {
		...
		fprintf(fpout, "1 1\n\n");
/* JR-022412 	} else if(strcmp(logical_block[iblk].model->name, "latch") == 0) { */
...
/* JR-022412 				}	*/
					...
					for(j = 0; j < port->size; j++) {
						if(logical_block[iblk].reset_net_tnode != NULL) {
							fprintf(fpout, "tnode_%d 0\n\n", logical_block[iblk].reset_net_tnode->index);
						} else {
							assert(0);
						}
					}
...
/* JR-022412 				}	*/
					...
					assert(port->size == 1);
					if(logical_block[iblk].reset_net_tnode != NULL) {
						fprintf(fpout, "\\\n%s=tnode_%d ", port->name, logical_block[iblk].reset_net_tnode->index);
					} else {
						fprintf(fpout, "\\\n%s=unconn ", port->name);
					}
					i++;
