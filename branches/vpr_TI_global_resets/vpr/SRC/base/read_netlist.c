...
static void processPorts(INOUTP ezxml_t Parent, INOUTP t_pb* pb, INOUTP t_rr_node *rr_graph,
					   INOUTP int *ncount, INOUTP struct s_hash **nhash);


static void processPb(INOUTP ezxml_t Parent, INOUTP t_pb* pb, INOUTP t_rr_node *rr_graph,
					   INOUTP int *ncount, INOUTP struct s_hash **nhash, INOUTP int *num_primitives);
static void processComplexBlock(INOUTP ezxml_t Parent, INOUTP t_block *cb, INP int index, 
					   INOUTP int *ncount, INOUTP struct s_hash **nhash, 
...
/* JR-022412 */
		nlist[curr_net->index].is_clock = FALSE;
		nlist[curr_net->index].is_reset = FALSE;
...
static void processPorts(INOUTP ezxml_t Parent, INOUTP t_pb* pb, t_rr_node *rr_graph,
					   INOUTP int *ncount, INOUTP struct s_hash **nhash) {

/* JR-022412 	int i, j, in_port, out_port, clock_port, num_tokens; */
	int i, j, in_port, out_port, clock_port, reset_port, num_tokens;
	ezxml_t Cur, Prev;
	const char *Prop;
	char **pins;
...
/* JR-022412 			in_port = out_port = clock_port = 0; */
			in_port = out_port = clock_port = reset_port = 0;
			found = FALSE;
...
/* JR-022412 				if(pb->pb_graph_node->pb_type->ports[i].is_clock && pb->pb_graph_node->pb_type->ports[i].type == IN_PORT) { 		*/
/* 	 					clock_port++; 													*/
/* 	 				} else if(!pb->pb_graph_node->pb_type->ports[i].is_clock && pb->pb_graph_node->pb_type->ports[i].type == IN_PORT) { 	*/
/* 	 					in_port++; 													*/
				if(pb->pb_graph_node->pb_type->ports[i].type == IN_PORT && !pb->pb_graph_node->pb_type->ports[i].is_clock && !pb->pb_graph_node->pb_type->ports[i].is_reset) {
					in_port++;
				} else if(pb->pb_graph_node->pb_type->ports[i].type == IN_PORT && pb->pb_graph_node->pb_type->ports[i].is_clock) {
					clock_port++;
				} else if(pb->pb_graph_node->pb_type->ports[i].type == IN_PORT && pb->pb_graph_node->pb_type->ports[i].is_reset) {
					reset_port++;
				} else if(pb->pb_graph_node->pb_type->ports[i].type == OUT_PORT) {
					out_port++;
				}
			}	
			if(!found) {
...
/* JR-022412 				if(num_tokens != pb->pb_graph_node->num_clock_pins[clock_port]) {  */
/* 	 					printf(ERRTAG "[Line %d] Incorrect # pins %d found for port %s for pb %s[%d]\n", Cur->line, num_tokens, Prop, pb->pb_graph_node->pb_type->name, pb->pb_graph_node->placement_index); */
				if(0 == strcmp(Prop, "clock")) {
					if(num_tokens != pb->pb_graph_node->num_clock_pins[clock_port]) {
						printf(ERRTAG "[Line %d] Incorrect # pins %d found for port %s for pb %s[%d]\n", Cur->line, num_tokens, Prop, pb->pb_graph_node->pb_type->name, pb->pb_graph_node->placement_index);
						exit(1);
					}
				} else if(0 == strcmp(Prop, "reset")) {
					if(num_tokens != pb->pb_graph_node->num_reset_pins[reset_port]) {
						printf(ERRTAG "[Line %d] Incorrect # pins %d found for port %s for pb %s[%d]\n", Cur->line, num_tokens, Prop, pb->pb_graph_node->pb_type->name, pb->pb_graph_node->placement_index);
						exit(1);
					}
				}
...
/* 								rr_node_index = pb->pb_graph_node->clock_pins[clock_port][i].pin_count_in_cluster; */
						} else if(0 == strcmp(Prop, "clock")) {
							rr_node_index = pb->pb_graph_node->clock_pins[clock_port][i].pin_count_in_cluster;
						} else if(0 == strcmp(Prop, "reset")) {
							rr_node_index = pb->pb_graph_node->reset_pins[reset_port][i].pin_count_in_cluster;
						}
			...
			INP char **circuit_globals)
{
	...
	*ext_ncount = 0;
	...
	for(i = 0; i < num_blocks; i++) {
		ipin = 0;
/* JR-022412 		if(block_list[i].type->pb_type->num_input_pins + 	*/
...
/* 	 		} 							*/
		if(block_list[i].type->pb_type->num_input_pins + 
			...
			block_list[i].type->pb_type->num_reset_pins != block_list[i].type->num_pins / block_list[i].type->capacity) {
			assert(0);
		}
			

		/* First determine nets external to complex blocks */
/* JR-022412 		assert(block_list[i].type->pb_type->num_input_pins + 	*/
/*  				block_list[i].type->pb_type->num_output_pins +  */
/* 	 			block_list[i].type->pb_type->num_clock_pins == block_list[i].type->num_pins / block_list[i].type->capacity); */
...
/* JR-022412 */
		for(j = 0; j < block_list[i].pb->pb_graph_node->num_reset_ports; j++) {
			for(k = 0; k < block_list[i].pb->pb_graph_node->num_reset_pins[j]; k++) {
				pb_graph_pin = &block_list[i].pb->pb_graph_node->reset_pins[j][k];
				assert(pb_graph_pin->pin_count_in_cluster == ipin);
				if(rr_graph[pb_graph_pin->pin_count_in_cluster].net_num != OPEN) {
					block_list[i].nets[ipin] = add_net_to_hash(ext_nhash, nlist[rr_graph[pb_graph_pin->pin_count_in_cluster].net_num].name, ext_ncount);
				} else {
...
/* JR-022412 */
					nets[*curr_net].is_clock = TRUE;
					nets[*curr_net].is_reset = FALSE;
					size = strlen(pb_graph_node->pb_type->name) + 
						pb_graph_node->placement_index/10 + 
...
/* JR-022412 */
		for(i = 0; i < pb_graph_node->num_reset_ports; i++) {
			for(j = 0; j < pb_graph_node->num_reset_pins[i]; j++) {
				if(rr_graph[pb_graph_node->reset_pins[i][j].pin_count_in_cluster].net_num != OPEN) {
					load_internal_cb_rr_graph_net_nums(&rr_graph[pb_graph_node->reset_pins[i][j].pin_count_in_cluster], rr_graph, nets, curr_net, &temp);
					assert(temp == nets[*curr_net].num_sinks);
					temp = 0;
					nets[*curr_net].is_global = TRUE;
					nets[*curr_net].is_clock = FALSE;
					nets[*curr_net].is_reset = TRUE;
					size = strlen(pb_graph_node->pb_type->name) + 
						pb_graph_node->placement_index/10 + 
						i / 10 + j /10 + pb_graph_node->reset_pins[i][j].pin_count_in_cluster/10 + 
						26;
					nets[*curr_net].name = my_calloc(size, sizeof(char));
					sprintf(nets[*curr_net].name, "%s[%d].reset[%d][%d].pin[%d]", pb_graph_node->pb_type->name,
						pb_graph_node->placement_index, i, j, pb_graph_node->reset_pins[i][j].pin_count_in_cluster);
					(*curr_net)++;
				}
			}
		}
...
/* JR-022412 */
		for(i = 0; i < pb_graph_node->num_reset_ports; i++) {
			for(j = 0; j < pb_graph_node->num_reset_pins[i]; j++) {
				if(rr_graph[pb_graph_node->reset_pins[i][j].pin_count_in_cluster].net_num != OPEN) {
					if(pass == 2) {
						num_sinks = count_sinks_internal_cb_rr_graph_net_nums(&rr_graph[pb_graph_node->reset_pins[i][j].pin_count_in_cluster], rr_graph);
						top_level->local_nets[top_level->num_local_nets].num_sinks = num_sinks;
						top_level->local_nets[top_level->num_local_nets].node_block = my_calloc(num_sinks, sizeof(int));
						top_level->local_nets[top_level->num_local_nets].node_block_port = my_calloc(num_sinks, sizeof(int));
						top_level->local_nets[top_level->num_local_nets].node_block_pin = my_calloc(num_sinks, sizeof(int));
...
/* JR-022412 */
		for(i = 0; i < pb->pb_graph_node->num_reset_ports && const_gen == TRUE; i++) {
			for(j = 0; j < pb->pb_graph_node->num_reset_pins[i] && const_gen == TRUE; j++) {
				if(rr_graph[pb->pb_graph_node->reset_pins[i][j].pin_count_in_cluster].net_num != OPEN) {
					const_gen = FALSE;
				}
			}
		}
...
/* JR-022412 */
	logical_block[iblk].reset_net = saved_logical_blocks[i].reset_net;
/* JR-022412 */
	logical_block[iblk].reset_net_tnode = NULL;
