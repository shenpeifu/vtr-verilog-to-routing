...
static void add_lut(int doall,
	t_model *logic_model);
/* JR-022412 static void add_latch(int doall, INP t_model *latch_model); */
static void add_latch(int doall, INP t_model *latch_model, INP t_model *latch_reset_model);
...
/* JR-022412 					OUTP t_model** logic_model, OUTP t_model** latch_model); */
						OUTP t_model** logic_model, OUTP t_model** latch_model, OUTP t_model** latch_model_reset );
	  ...
	  t_model *library_models)
{
	...
	boolean done;
	boolean add_truth_table;
/* JR-022412 	t_model *inpad_model, *outpad_model, *logic_model, *latch_model; */
	t_model *inpad_model, *outpad_model, *logic_model, *latch_model, *latch_reset_model;
...
/* JR-022412 	load_default_models(library_models, &inpad_model, &outpad_model, &logic_model, &latch_model); */
	load_default_models(library_models, &inpad_model, &outpad_model, &logic_model, &latch_model, &latch_reset_model);
 ...
 * LUTs and latches third, subckts last.  Just makes the output netlist more readable. */
	    ...
	    for(pass = 0; pass <= 4; pass++) 
		{
			...
			{
/* JR-022412 				get_tok(buffer, pass, doall, &done, &add_truth_table, inpad_model, outpad_model, logic_model, latch_model, user_models); */
				get_tok(buffer, pass, doall, &done, &add_truth_table, inpad_model, outpad_model, logic_model, latch_model, latch_reset_model, user_models);
			}
...
/* JR-022412 */
			vpack_net[i].is_clock = FALSE;
			vpack_net[i].is_reset = FALSE;
		}
...
/* JR-022412     if(strcmp(ptr, ".latch") == 0) */
    if((strcmp(ptr, ".latch") == 0) ||
       (strcmp(ptr, ".latch_reset") == 0))
	{
		*add_truth_table = FALSE;
	    if(pass == 3)
		{
/* JR-022412 		    add_latch(doall, latch_model); */
		    add_latch(doall, latch_model, latch_reset_model);
		}
	    else
		{
		    dum_parse(buffer);
...
/* JR-022412 			add_vpack_net(saved_names[j], RECEIVER, num_logical_blocks - 1, 0, j, FALSE, doall); */
			add_vpack_net(saved_names[j], RECEIVER, num_logical_blocks - 1, 0, j, FALSE, FALSE, FALSE, doall);
		free_matrix(saved_names, 0, logic_model->inputs->size, 0, sizeof(char));
	    return;
	}
...
/* JR-022412 */
	logical_block[num_logical_blocks - 1].reset_net = OPEN;
...
/* JR-022412 		logical_block[num_logical_blocks - 1].input_nets[0][i] = add_vpack_net(saved_names[i], RECEIVER, num_logical_blocks - 1, 0, i, FALSE, doall); */
	logical_block[num_logical_blocks - 1].input_nets[0][i] = add_vpack_net(saved_names[i], RECEIVER, num_logical_blocks - 1, 0, i, FALSE, FALSE, FALSE, doall);
/* JR-022412 */
    logical_block[num_logical_blocks - 1].output_nets[0][0] = add_vpack_net(saved_names[output_net_index], DRIVER, num_logical_blocks - 1, 0, 0, FALSE, FALSE, FALSE, doall);
...
static void
/* JR-022412 add_latch(int doall, INP t_model *latch_model) */
add_latch(int doall, INP t_model *latch_model, INP t_model *latch_reset_model)
{
...
/* JR-022412 * .latch <input> <output> <type (latch on)> <control (clock)> <init_val> *
 * .latch <input> <output> <type (latch on)> <control (clock)> <init_val> <reset> *
 * The latch pins are in .nets 0 to 2 in the order: Q D CLOCK.            */

/* JR-022412    char *ptr, buf[BUFSIZE], saved_names[6][BUFSIZE]; */
    char *ptr, buf[BUFSIZE], saved_names[7][BUFSIZE];
    int i;
...
/* JR-022412 */
	    add_vpack_net(saved_names[0], RECEIVER, num_logical_blocks - 1, 0, 0, FALSE, FALSE, FALSE, doall);	/* D */
	    add_vpack_net(saved_names[1], DRIVER, num_logical_blocks - 1, 0, 0, FALSE, FALSE, FALSE, doall);	/* Q */
	    add_vpack_net(saved_names[3], RECEIVER, num_logical_blocks - 1, 0, 0, TRUE, TRUE, FALSE, doall);	/* Clock */
/* JR-022412 */
	    if(*saved_names[5])
	        add_vpack_net(saved_names[5], RECEIVER, num_logical_blocks - 1, 0, 0, TRUE, FALSE, TRUE, doall);	/* Reset */
	    return;
	}

/* JR-022412 	logical_block[num_logical_blocks - 1].model = latch_model; */
	if(*saved_names[5]) {
		logical_block[num_logical_blocks - 1].model = latch_reset_model;
	} else {
...
/* JR-022412 */
    logical_block[num_logical_blocks - 1].output_nets[0][0] = add_vpack_net(saved_names[1], DRIVER, num_logical_blocks - 1, 0, 0, FALSE, FALSE, FALSE, doall);
    logical_block[num_logical_blocks - 1].input_nets[0][0] = add_vpack_net(saved_names[0], RECEIVER, num_logical_blocks - 1, 0, 0, FALSE, FALSE, FALSE, doall);
    logical_block[num_logical_blocks - 1].clock_net = add_vpack_net(saved_names[3], RECEIVER, num_logical_blocks - 1, 0, 0, TRUE, TRUE, FALSE, doall);
/* JR-022412 */
    if(*saved_names[5])
        logical_block[num_logical_blocks - 1].reset_net = add_vpack_net(saved_names[5], RECEIVER, num_logical_blocks - 1, 0, 0, TRUE, FALSE, TRUE, doall);
    else
        logical_block[num_logical_blocks - 1].reset_net = OPEN;
...
/* JR-022412 				add_vpack_net(circuit_signal_name[subckt_index_signals], RECEIVER, num_logical_blocks - 1, 0, 0, FALSE, doall); */
				add_vpack_net(circuit_signal_name[subckt_index_signals], RECEIVER, num_logical_blocks - 1, 0, 0, FALSE, FALSE, FALSE, doall);
			}
...
/* JR-022412		assert(port == NULL || (port->is_clock && port->next == NULL)); 	*/
		assert(port == NULL || (port->is_clock && port->next == NULL) || (port->is_reset && port->next == NULL));
...
/* JR-022412 							add_vpack_net(circuit_signal_name[i], RECEIVER, num_logical_blocks - 1, port->index, my_atoi(pin_number), TRUE, doall); */
							add_vpack_net(circuit_signal_name[i], RECEIVER, num_logical_blocks - 1, port->index, my_atoi(pin_number), TRUE, TRUE, FALSE, doall);
/* JR-022412 */
					} else if(port->is_reset) {
						assert(logical_block[num_logical_blocks-1].reset_net == OPEN);
						assert(my_atoi(pin_number) == 0);
						logical_block[num_logical_blocks-1].reset_net = 
/* JR-022412 							add_vpack_net(circuit_signal_name[i], RECEIVER, num_logical_blocks - 1, port->index, my_atoi(pin_number), TRUE, doall); */
							add_vpack_net(circuit_signal_name[i], RECEIVER, num_logical_blocks - 1, port->index, my_atoi(pin_number), TRUE, FALSE, TRUE, doall);
					} else {
						logical_block[num_logical_blocks-1].input_nets[port->index][my_atoi(pin_number)] = 
/* JR-022412 							add_vpack_net(circuit_signal_name[i], RECEIVER, num_logical_blocks - 1, port->index, my_atoi(pin_number), FALSE, doall); */
							add_vpack_net(circuit_signal_name[i], RECEIVER, num_logical_blocks - 1, port->index, my_atoi(pin_number), FALSE, FALSE, FALSE, doall);
						input_net_count++;
					}
				}
				port = port->next;
...
/* JR-022412 						add_vpack_net(circuit_signal_name[i], DRIVER, num_logical_blocks - 1, port->index, my_atoi(pin_number), FALSE, doall); */
						add_vpack_net(circuit_signal_name[i], DRIVER, num_logical_blocks - 1, port->index, my_atoi(pin_number), FALSE, FALSE, FALSE, doall);
...
/* JR-022412 	    nindex = add_vpack_net(ptr, in_or_out, num_logical_blocks - 1, 0, 0, FALSE, doall); */
	    nindex = add_vpack_net(ptr, in_or_out, num_logical_blocks - 1, 0, 0, FALSE, FALSE, FALSE, doall);
...
/* JR-022412 */
			vpack_net[nindex].is_clock = is_clock;
			vpack_net[nindex].is_reset = is_reset;
		    return (nindex);
		}
	    prev_ptr = h_ptr;
	    h_ptr = h_ptr->next;
...
/* JR-022412 */
	...
	cur = library_models;
/* JR-022412 	logic_model = latch_model = NULL; */
	logic_model = latch_model = latch_reset_model = NULL;
	while(cur) {
...
/* JR-022412 		} */
		} else if(strcmp(cur->name, MODEL_LATCH_RESET) == 0) {
			latch_reset_model = cur;
			assert(latch_reset_model->inputs->size == 1);
		}
		cur = cur->next;
	}
...
/* JR-022412 		} */
		} else if (logical_block[i].model == latch_reset_model) {
			if(latch_reset_model == NULL)
				continue;
			inet = logical_block[i].input_nets[0][0];
			if (vpack_net[inet].num_sinks == 1 && logical_block[vpack_net[inet].node_block[0]].model == logic_model) {
				num_absorbable_latch++;
...
/* JR-022412 */
		fprintf(fp, "\treset net %d\n", logical_block[i].reset_net);
	}
...
/* JR-022412 					OUTP t_model** logic_model, OUTP t_model** latch_model) */
					OUTP t_model** logic_model, OUTP t_model** latch_model, OUTP t_model** latch_reset_model)
{
	t_model *cur_model;
	cur_model = library_models;
/* JR-022412 	*inpad_model = *outpad_model = *logic_model = *latch_model = NULL; */
	*inpad_model = *outpad_model = *logic_model = *latch_model = *latch_reset_model = NULL;
	while(cur_model) {
...
/* JR-022412 */
		} else if(strcmp(MODEL_LATCH_RESET, cur_model->name) == 0) {
			assert(cur_model->outputs->next == NULL);
			assert(cur_model->outputs->size == 1);
			*latch_reset_model = cur_model;
		} else {
			assert(0);
		}
		cur_model = cur_model->next;
...
/* JR-022412						check_net = logical_block[iblk].clock_net; 	*/
						if((logical_block[iblk].clock_net == OPEN) && (logical_block[iblk].reset_net == OPEN)) {
							check_net = logical_block[iblk].clock_net;
						} else if((logical_block[iblk].clock_net != OPEN) && (logical_block[iblk].reset_net == OPEN)) {
							check_net = logical_block[iblk].clock_net;
						} else if((logical_block[iblk].clock_net == OPEN) && (logical_block[iblk].reset_net != OPEN)) {
							check_net = logical_block[iblk].reset_net;
						} else if(logical_block[iblk].clock_net == i ) {
							check_net = logical_block[iblk].clock_net;
						} else if(logical_block[iblk].reset_net == i ) {
							check_net = logical_block[iblk].reset_net;
						} else {
							check_net = logical_block[iblk].clock_net;
						}
