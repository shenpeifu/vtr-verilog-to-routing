...
/* JR-022412 static int ext_input_rr_node_index, ext_output_rr_node_index, ext_clock_rr_node_index, max_ext_index; */
static int ext_input_rr_node_index, ext_output_rr_node_index, ext_clock_rr_node_index, ext_reset_rr_node_index, max_ext_index;
...
/* JR-022412 input_port = output_port = clock_port = 0; 	*/
	input_port = output_port = clock_port = reset_port = 0;
...
/* JR-022412			if(prim_port->is_clock) { 			*/
/* 					clock_port++; 				*/
/* 					assert(prim_port->type == IN_PORT); 	*/
/* 				} else if(prim_port->type == IN_PORT) { 	*/
/* 					input_port++; 				*/
			if(prim_port->type == IN_PORT && !prim_port->is_clock && !prim_port->is_reset) {
				input_port++;
			} else if(prim_port->type == IN_PORT && prim_port->is_clock) {
				clock_port++;
			} else if(prim_port->type == IN_PORT && prim_port->is_reset) {
				reset_port++;
	...
	if(ipin >= prim_port->num_pins) {
		...
		net_pin = OPEN;
/* JR-022412 		} else if(prim_port->type == IN_PORT) { */
		if(prim_port->type == IN_PORT && !prim_port->is_clock && !prim_port->is_reset) {
...
/* JR-022412 */
			...
			assert(net_pin != OPEN);
			net_rr_terminals[iblk_net][net_pin] = 
				primitive->pb_graph_node->reset_pins[reset_port][ipin].pin_count_in_cluster;
		} else if(prim_port->type == OUT_PORT) {
			i = 0;
			if(vpack_net[iblk_net].node_block[i] == ilogical_block &&
				vpack_net[iblk_net].node_block_port[i] == model_port->index &&
...
/* JR-022412 	int curr_ext_output, curr_ext_input, curr_ext_clock; */
	int curr_ext_output, curr_ext_input, curr_ext_clock, curr_ext_reset;

	curr_ext_input = ext_input_rr_node_index;
	curr_ext_output = ext_output_rr_node_index;
	curr_ext_clock = ext_clock_rr_node_index;
/* JR-022412 */
	curr_ext_reset = ext_reset_rr_node_index;
...
/* JR-022412 				net_rr_terminals[net_index][0] = curr_ext_clock;	*/
/* 	 				curr_ext_clock++; 					*/
				if(vpack_net[net_index].is_clock) {
					net_rr_terminals[net_index][0] = curr_ext_clock;
					curr_ext_clock++;
				} else if(vpack_net[net_index].is_reset) {
					net_rr_terminals[net_index][0] = curr_ext_reset;
					curr_ext_reset++;
				}
			} else {
...
/* JR-022412 		if( curr_ext_input > ext_output_rr_node_index || 	*/
/* 	 			curr_ext_output > ext_clock_rr_node_index || 	*/
/* 	 			curr_ext_clock > max_ext_index) { 		*/
		if( curr_ext_input > ext_output_rr_node_index || 
			curr_ext_output > ext_clock_rr_node_index ||
			curr_ext_clock > ext_reset_rr_node_index ||
			curr_ext_reset > max_ext_index) {
...
/* JR-022412 */
	for(i = 0; i < pb_graph_node->num_reset_ports; i++) {
		for(j = 0; j < pb_graph_node->num_reset_pins[i]; j++) {
			index = pb_graph_node->reset_pins[i][j].pin_count_in_cluster;
			rr_node[index].pb_graph_pin = &pb_graph_node->reset_pins[i][j];
			rr_node[index].fan_in = pb_graph_node->reset_pins[i][j].num_input_edges;
			rr_node[index].num_edges = pb_graph_node->reset_pins[i][j].num_output_edges;
			rr_node[index].pack_intrinsic_cost = 1 + (float)rr_node[index].num_edges / 5; /* need to normalize better than 5 */
			rr_node[index].edges = my_malloc(rr_node[index].num_edges * sizeof(int));
			rr_node[index].switches = my_calloc(rr_node[index].num_edges, sizeof(int));
			rr_node[index].net_num = OPEN;
			...
			for(k = 0; k < pb_graph_node->reset_pins[i][j].num_output_edges; k++) {
				/* TODO: Intention was to do bus-based implementation here */
				rr_node[index].edges[k] = pb_graph_node->reset_pins[i][j].output_edges[k]->output_pins[0]->pin_count_in_cluster;
				rr_node[index].switches[k] = arch->num_switches - 1;
				assert(pb_graph_node->reset_pins[i][j].output_edges[k]->num_output_pins == 1);
			}
			rr_node[index].type = INTRA_CLUSTER_EDGE;
			if(is_primitive) {
				rr_node[index].type = SINK;
...
/* JR-022412 					pb_type->num_output_pins + pb_type->num_clock_pins; */
					pb_type->num_output_pins + pb_type->num_clock_pins + pb_type->num_reset_pins;
...
/* JR-022412 */
	ext_reset_rr_node_index = pb_type->num_input_pins + pb_type->num_output_pins + pb_type->num_clock_pins + pb_graph_node->total_pb_pins;

/* JR-022412 	max_ext_index = pb_type->num_input_pins + pb_type->num_output_pins + pb_type->num_clock_pins + pb_graph_node->total_pb_pins; */
	max_ext_index = pb_type->num_input_pins + pb_type->num_output_pins + pb_type->num_clock_pins + pb_type->num_reset_pins + pb_graph_node->total_pb_pins;
...
/* JR-022412 */
	for(i = 0; i < pb_type->num_reset_pins; i++) {
		index = i + pb_type->num_input_pins + pb_type->num_output_pins + pb_type->num_clock_pins + pb_graph_node->total_pb_pins;
		rr_node[index].type = SOURCE;
		rr_node[index].fan_in = 0;
		rr_node[index].num_edges = pb_type->num_reset_pins;
		rr_node[index].pack_intrinsic_cost = 1 + (float)rr_node[index].num_edges / 5; /* need to normalize better than 5 */
		rr_node[index].edges = my_malloc(rr_node[index].num_edges * sizeof(int));
		rr_node[index].switches = my_calloc(rr_node[index].num_edges, sizeof(int));
		rr_node[index].capacity = 1;
...
/* JR-022412 */
	...
	for(i = 0; i < pb_graph_node->num_reset_ports; i++) {
		for(j = 0; j < pb_graph_node->num_reset_pins[i]; j++) {
			for(k = 0; k < pb_type->num_reset_pins; k++) {
				index = k + pb_type->num_input_pins + pb_type->num_output_pins + pb_type->num_clock_pins + pb_graph_node->total_pb_pins;
				pb_graph_rr_index = pb_graph_node->reset_pins[i][j].pin_count_in_cluster;
				rr_node[index].edges[ipin] = pb_graph_rr_index;	
				rr_node[index].switches[ipin] = arch->num_switches - 1;
			}
			ipin++;
...
/* 				} 									*/
			if(!port->is_clock && !port->is_reset) {
				...
				iblk_net = logical_block[iblock].reset_net;
			}
