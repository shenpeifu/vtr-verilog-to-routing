...
/* JR-022412 */
					  ...
					  assert(port->size == 1 && port->index == 0 && ipin == 0);
					  if(logical_block[index].reset_net == OPEN)
						continue;
					  logical_block[index].reset_net =
						net_remap[logical_block[index].reset_net];
				  } else {
...
/* JR-022412 */
/*===========================================================================//
// Function       : alloc_and_load_is_reset
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
...
boolean *alloc_and_load_is_reset () {
	...
	is_reset = (boolean *) my_calloc (num_logical_nets, sizeof(boolean));

	/* Want to identify all the reset nets.  */

	for (bnum=0;bnum<num_logical_blocks;bnum++) {
		if(logical_block[bnum].reset_net != OPEN) {
			reset_net = logical_block[bnum].reset_net;
			is_reset[reset_net] = TRUE;
		}
	}
	return (is_reset);
}
