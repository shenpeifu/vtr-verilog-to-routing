...
/* JR-022412 static boolean inputs_outputs_models_and_clocks_feasible (INP enum e_packer_algorithm packer_algorithm, int iblk, boolean *is_clock, t_pb *cur_pb); */
static boolean inputs_outputs_models_and_clocks_feasible (INP enum e_packer_algorithm packer_algorithm, int iblk, boolean *is_clock, boolean *is_reset, t_pb *cur_pb);
...
/* JR-022412									(enum e_packer_algorithm packer_algorithm, int iblk, boolean *is_clock, t_pb *cur_pb), 	*/
									(enum e_packer_algorithm packer_algorithm, int iblk, boolean *is_clock, boolean *is_reset, t_pb *cur_pb), 
...
/* JR-022412 */
static boolean cluster_unique_global_feasible( int iblock, t_pb* pb, t_pb* parent_pb, boolean is_clock, boolean is_reset );
static boolean cluster_sibling_global_feasible( int iblock, t_pb* pb, boolean is_clock, boolean is_reset );
static boolean cluster_netpins_global_feasible( int iblock, int inet, t_pb* pb, boolean is_clock, boolean is_reset );

/*****************************************/
/*globally accessable function*/
void do_clustering (const t_arch *arch, int num_models, boolean global_clocks,
/* JR-022412        boolean *is_clock, boolean hill_climbing_flag,	*/
       boolean *is_clock, boolean *is_reset, boolean hill_climbing_flag, 
...
/* JR-022412	update_cluster_stats (istart, num_clb, is_clock, global_clocks, alpha, beta, timing_driven, connection_driven); 	*/
	update_cluster_stats (istart, num_clb, is_clock, is_reset, global_clocks, alpha, beta, timing_driven, connection_driven);
	num_clb++;
...
/* JR-022412		update_cluster_stats (next_blk, num_clb - 1, is_clock, global_clocks, */
		update_cluster_stats (next_blk, num_clb - 1, is_clock, is_reset, global_clocks, 
...
/* JR-022412 output_clustering(	clb, num_clb, global_clocks, is_clock, out_fname, FALSE); */
 output_clustering(	clb, num_clb, global_clocks, is_clock, is_reset, out_fname, FALSE);
#ifdef DUMP_BLIF_ECHO
 if(!hack_frac_lut_no_legality) /* must have routing graph before dumping blif */
/* JR-022412	output_blif(	clb, num_clb, global_clocks, is_clock, "post_packing_blif.echo", FALSE); */
	output_blif(	clb, num_clb, global_clocks, is_clock, is_reset, "post_packing_blif.echo", FALSE);
#endif
 ...
 * pins used - the number which connect (internally) to the outputs.   */
 ...
 while(port) {
/* JR-022412	 if(port->is_clock == FALSE) { 	*/
	 if(!port->is_clock && !port->is_reset) {
 ...
 * face as the other feasibility checkers.                          */
 ...
 while(temp_pb && clocks_feasible && outputs_feasible) {
	...
	if(clocks_avail == NOT_VALID) {
	 ...
	 port = logical_block[iblk].model->inputs;
	 while(port) {
/* JR-022412		 if(port->is_clock == TRUE) { 	*/
		 if(port->is_clock || port->is_reset) {
			 port = port->next;
			 continue;
		 }
...
/* JR-021412 	if( !cluster_unique_clock_feasible( iblk, cur_pb, 0 ) || !cluster_sibling_clock_feasible( iblk, cur_pb )) {	*/
/* 			return FALSE; 												*/
/* 	        } 														*/
	if( !cluster_unique_global_feasible( iblk, cur_pb, 0, TRUE, TRUE ) || 
            !cluster_sibling_global_feasible( iblk, cur_pb, TRUE, TRUE )) {
		return FALSE;
        }
	...
	if(cur_pb_type->num_modes == 0) {
		if(hack_force_safe_latch) {
			...
			*/
/* JR-022412 			if(strcmp(logical_block[iblk].model->name, "latch") == 0) { */
			if((strcmp(logical_block[iblk].model->name, "latch") == 0) ||
			   (strcmp(logical_block[iblk].model->name, "latch_reset") == 0)) {
...
/* JR-022412					if(port->dir == IN_PORT && !port->is_clock) { 	*/

					if(port->dir == IN_PORT && !port->is_clock && !port->is_reset) {
						if(memory_class_pb) {
...
/* 							} 											*/
/* 						} 												*/
					} else if(port->dir == IN_PORT && port->is_clock) {
						assert(j == 0);
						if(memory_class_pb) {
							if(logical_block[iblk].clock_net != logical_block[sibling_memory_blk].clock_net) {
								return FALSE;
					...
					} else if(port->dir == IN_PORT && port->is_reset) {
						assert(j == 0);
						if(memory_class_pb) {
							if(logical_block[iblk].reset_net != logical_block[sibling_memory_blk].reset_net) {
								return FALSE;
							}
						}
						if(logical_block[iblk].reset_net != OPEN && j >= cur_pb_type->ports[i].num_pins) {
							return FALSE;
						}
					}
				}
 ...
 * benchmarks actually generate local clocks (all come from pads).  */
 ...
 while(port) {
	 for(ipin = 0; ipin < port->size; ipin++) {
		 ...
		 if (inet != OPEN) {
/* JR-022412			if (!is_clock[inet] || !global_clocks) 	*/
			if (!is_clock[inet] || !is_reset[inet] || !global_clocks) 
...
/* JR-022412				if (cur_pb->pb_stats.num_pins_of_net_in_pb[inet] > 1 && !is_clock[inet]) 	*/
				if (cur_pb->pb_stats.num_pins_of_net_in_pb[inet] > 1 && !is_clock[inet] && !is_reset[inet])
 ...
 while(temp_pb && inputs_feasible) {
	 ...
	 while(port) {
		/* Outputs that connect to internal blocks free up an input pin. */
		 for(ipin = 0; ipin < port->size; ipin++) {
			 output_net = logical_block[iblk].output_nets[port->index][ipin];
/* JR-022412			 if (output_net != OPEN && temp_pb->pb_stats.num_pins_of_net_in_pb[output_net] != 0 && !is_clock[output_net]) 	*/
			 if (output_net != OPEN && temp_pb->pb_stats.num_pins_of_net_in_pb[output_net] != 0 && !is_clock[output_net] && !is_reset[output_net])
				inputs_avail++;
		 }
		 port = port->next;
	 }
...
/* JR-022412 */
												INP boolean *is_reset, 
/* JR-022412												INP boolean (*is_feasible) (enum e_packer_algorithm packer_algorithm, int iblk, boolean *is_clock, t_pb *cur_pb), */
												INP boolean (*is_feasible) (enum e_packer_algorithm packer_algorithm, int iblk, boolean *is_clock, boolean *is_reset, t_pb *cur_pb), 
												INP enum e_gain_type gain_mode) {
...
/* JR-022412						if (is_feasible (packer_algorithm, iblk, is_clock, cur_pb)) { 	*/
						if (is_feasible (packer_algorithm, iblk, is_clock, is_reset, cur_pb)) {
...
/* JR-022412 best_block = get_highest_gain_logical_block (packer_algorithm, cur_pb, is_clock, inputs_outputs_models_and_clocks_feasible, */
 best_block = get_highest_gain_logical_block (packer_algorithm, cur_pb, is_clock, is_reset, inputs_outputs_models_and_clocks_feasible,
						 NOT_HILL_CLIMBING);
...
/* JR-022412	int inport, outport, clockport; */
	int inport, outport, clockport, resetport;
...
/* JR-022412		inport = outport = clockport = 0; */
		inport = outport = clockport = resetport = 0;	
		ipin = 0;
		/* Assume top-level pb and clb share a one-to-one connection */
		for(i = 0; i < pb_type->num_ports; i++) {
/* JR-022412			if(!pb_type->ports[i].is_clock && pb_type->ports[i].type == IN_PORT) { 	*/
			if(!pb_type->ports[i].is_clock && !pb_type->ports[i].is_reset && pb_type->ports[i].type == IN_PORT) {
...
/* 					clockport++; 										*/
/* 				} 												*/
			} else if(pb_type->ports[i].is_clock && pb_type->ports[i].type == IN_PORT) {
				for(j = 0; j < pb_type->ports[i].num_pins; j++) {
					iclass = clb[i_clb].type->pin_class[ipin];
					assert(clb[i_clb].type->class_inf[iclass].type == RECEIVER);
					assert(clb[i_clb].type->is_global_pin[ipin]);
				...
				clockport++;
			} else if(pb_type->ports[i].is_reset && pb_type->ports[i].type == IN_PORT) {
				for(j = 0; j < pb_type->ports[i].num_pins; j++) {
					iclass = clb[i_clb].type->pin_class[ipin];
					assert(clb[i_clb].type->class_inf[iclass].type == RECEIVER);
					assert(clb[i_clb].type->is_global_pin[ipin]);
					node_index = pb->pb_graph_node->reset_pins[resetport][j].pin_count_in_cluster;
					clb[i_clb].nets[ipin] = rr_node[node_index].net_num;
					ipin++;
				}
				resetport++;
...
/* JR-022412 */
/*===========================================================================//
// Function       : cluster_unique_global_feasible
// Description    : Searches all primitive nodes associated with the given
//                  physical block to determine if the given global clock or 
//                  reset net is 'feasible'. This implementation currently 
//                  ASSUMES only one clock and only one reset is defined per 
//                  logic block. Therefore, any primitive nodes found with a 
//                  different net will result in a feasibility failure.
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 02/24/12 jeffr : Original
//===========================================================================*/
static boolean cluster_unique_global_feasible( int iblock, t_pb* pb, t_pb* parent_pb, boolean is_clock, boolean is_reset )
{
   ...
   if( pb && parent_pb )
   {
      if(( is_clock && logical_block[iblock].clock_net != OPEN ) ||
         ( is_reset && logical_block[iblock].reset_net != OPEN ))
      {
         /* Handle case where we apply current physical block, then travese children */
         if( pb->logical_block != OPEN )
         {
            /* We have arrived at a primitive node, ready to test global net */
            int jblock = pb->logical_block;

            if(( is_clock ) &&
               ( logical_block[jblock].clock_net != OPEN ) &&
               ( logical_block[jblock].clock_net != logical_block[iblock].clock_net ))
            {
               is_feasible = FALSE;
            }
            if(( is_reset ) &&
               ( logical_block[jblock].reset_net != OPEN ) &&
               ( logical_block[jblock].reset_net != logical_block[iblock].reset_net ))
            {
               is_feasible = FALSE;
            }
         }
         else if( pb->child_pbs )
         {
            /* We need to travese (recurse) children until reaching primitive nodes */
            const t_pb_type *pb_type = pb->pb_graph_node->pb_type;
            int mode = pb->mode;

            int i;
            for( i = 0; i < pb_type->modes[mode].num_pb_type_children; ++i ) 
            {
               if( !pb->child_pbs[i] )
                  continue;

               int j;
               for( j = 0; j < pb_type->modes[mode].pb_type_children[i].num_pb; ++j )
               {
                  t_pb* child_pb = &pb->child_pbs[i][j];
   	       if( !child_pb )
                     continue;

                  is_feasible = cluster_unique_global_feasible( iblock, child_pb, parent_pb, is_clock, is_reset );
                  if( !is_feasible )
                     break;
               }
               if( !is_feasible )
   ...
   if( pb && !parent_pb )
   {
      if(( is_clock && logical_block[iblock].clock_net != OPEN ) ||
         ( is_reset && logical_block[iblock].reset_net != OPEN ))
      {
         /* Handle case where we travese to parent-most node to start recursion */
         parent_pb = pb;
         while( parent_pb->parent_pb )
            parent_pb = parent_pb->parent_pb;

         is_feasible = cluster_unique_global_feasible( iblock, parent_pb, parent_pb, is_clock, is_reset );
      }
   }
   return( is_feasible );
}
...
/* JR-022412 */
/*===========================================================================//
// Function       : cluster_sibling_global_feasible
// Description    : Searches all sibling logical blocks with respect to the 
//                  given logical block to determine if each sibling's global
//                  net is 'feasible' (if any).
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 02/24/12 jeffr : Original
...
static boolean cluster_sibling_global_feasible( int iblock, t_pb* pb, boolean is_clock, boolean is_reset )
{
   ...
   {
      /* Find sibling logical blocks based on input port net(s) node blocks */
      t_model_ports *input_port = logical_block[iblock].model->inputs;
      while( input_port && is_feasible )
      {
         int ipin;
         for( ipin = 0; ipin < input_port->size; ++ipin ) 
         {
            int input_net = logical_block[iblock].input_nets[input_port->index][ipin];

            is_feasible = cluster_netpins_global_feasible( iblock, input_net, pb, is_clock, is_reset );
            if( !is_feasible )
               break;
         }
   ...
   if( logical_block[iblock].type == VPACK_COMB )
   {
      /* Find sibling logical blocks based on output port net(s) node blocks */
      t_model_ports *output_port = logical_block[iblock].model->outputs;
      while( output_port && is_feasible )
      {
         int ipin;
         for( ipin = 0; ipin < output_port->size; ++ipin ) 
         {
            int output_net = logical_block[iblock].output_nets[output_port->index][ipin];

            is_feasible = cluster_netpins_global_feasible( iblock, output_net, pb, is_clock, is_reset );
            if( !is_feasible )
               break;
         }
...
/* JR-022412 */
/*===========================================================================//
// Function       : cluster_netpins_global_feasible
// Description    : Searches all logical blocks assoicated with a given net 
//                  to determine if the given global net is 'feasible'.
// Author         : Jeff Rudolph
//---------------------------------------------------------------------------//
// Version history
// 02/24/12 jeffr : Original
...
static boolean cluster_netpins_global_feasible( int iblock, int inet, t_pb* pb, boolean is_clock, boolean is_reset )
{
   ...
   if(( iblock != OPEN ) && 
      ( inet != OPEN ))
   {
      /* Iterate net's output and input pins to test net's logical block feasibility */
      int npins = 1 + vpack_net[inet].num_sinks;
      int ipin;
      for( ipin = 0; ipin < npins; ++ipin )
      {
	 int jblock = vpack_net[inet].node_block[ipin];
	 if( iblock == jblock )
         {
            continue;
         }

         if(( logical_block[jblock].type != VPACK_LATCH ) &&
            ( logical_block[jblock].type != VPACK_COMB ))
         {
            continue;
         }

         if( !cluster_unique_global_feasible( jblock, pb, 0, is_clock, is_reset ))
         {
            is_feasible = FALSE;
            break;
         }
