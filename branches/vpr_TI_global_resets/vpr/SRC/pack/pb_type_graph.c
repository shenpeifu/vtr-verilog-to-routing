...
static void alloc_and_load_pb_graph(INOUTP t_pb_graph_node *pb_graph_node, 
									...
									INP const t_pb_type *pb_type, 
									INP int index) {
/* JR-022412 	int i, j, k, i_input, i_output, i_clockport; */
	int i, j, k, i_input, i_output, i_clockport, i_resetport;
...
/* JR-022412 */
	pb_graph_node->num_reset_ports = 0;

	/* Generate ports for pb graph node */
	for(i = 0; i < pb_type->num_ports; i++) {
/* JR-022412		if(pb_type->ports[i].type == IN_PORT && !pb_type->ports[i].is_clock) { 		*/
...
/* JR-022412 */
	pb_graph_node->num_reset_pins = my_calloc(pb_graph_node->num_reset_ports, sizeof(int));
...
/* JR-022412 */
	pb_graph_node->reset_pins = my_calloc(pb_graph_node->num_reset_ports, sizeof(t_pb_graph_pin*));
	
/* JR-022412 	i_input = i_output = i_clockport = 0; */
	i_input = i_output = i_clockport = i_resetport = 0;
	for(i = 0; i < pb_type->num_ports; i++) {
		if(pb_type->ports[i].model_port) {
			assert(pb_type->num_modes == 0);
		} else {
/* JR-022412			assert(pb_type->num_modes != 0 || pb_type->ports[i].is_clock); 	*/
			assert(pb_type->num_modes != 0 || pb_type->ports[i].is_clock || pb_type->ports[i].is_reset);
		}
/* JR-022412		if(pb_type->ports[i].type == IN_PORT && !pb_type->ports[i].is_clock) { 	*/
		if(pb_type->ports[i].type == IN_PORT && !pb_type->ports[i].is_clock && !pb_type->ports[i].is_reset) {
...
/* JR-022412 */
			...
			pb_graph_node->reset_pins[i_resetport] = my_calloc(pb_type->ports[i].num_pins, sizeof(t_pb_graph_pin));
			pb_graph_node->num_reset_pins[i_resetport] = pb_type->ports[i].num_pins;
			for(j = 0; j < pb_type->ports[i].num_pins; j++) {
				pb_graph_node->reset_pins[i_resetport][j].input_edges = NULL;
				pb_graph_node->reset_pins[i_resetport][j].num_input_edges = 0;
				pb_graph_node->reset_pins[i_resetport][j].output_edges = NULL;
				pb_graph_node->reset_pins[i_resetport][j].num_output_edges = 0;
				pb_graph_node->reset_pins[i_resetport][j].pin_number = j;
				pb_graph_node->reset_pins[i_resetport][j].port = &pb_type->ports[i];
				pb_graph_node->reset_pins[i_resetport][j].parent_node = pb_graph_node;
				pb_graph_node->reset_pins[i_resetport][j].pin_count_in_cluster = pin_count_in_cluster;
				pb_graph_node->reset_pins[i_resetport][j].type = PB_PIN_NORMAL;			
				if(pb_graph_node->pb_type->blif_model != NULL) {
					pb_graph_node->reset_pins[i_resetport][j].type = PB_PIN_RESET;			
				}
				pin_count_in_cluster++;
			}
			i_resetport++;
...
/* JR-022412 */
	for(i = 0; i < pb_graph_node->num_reset_ports; i++) {
		for(j = 0; j < pb_graph_node->num_reset_pins[i]; j++) {
			if(pb_graph_node->reset_pins[i][j].pin_timing)
				free(pb_graph_node->reset_pins[i][j].pin_timing);
			if(pb_graph_node->reset_pins[i][j].pin_timing_del_max)
				free(pb_graph_node->reset_pins[i][j].pin_timing_del_max);
			if(pb_graph_node->reset_pins[i][j].input_edges)
				free(pb_graph_node->reset_pins[i][j].input_edges);
			if(pb_graph_node->reset_pins[i][j].output_edges)
				free(pb_graph_node->reset_pins[i][j].output_edges);
		}
		free(pb_graph_node->reset_pins[i]);		
	}
	free(pb_graph_node->input_pins);
	free(pb_graph_node->output_pins);
	free(pb_graph_node->clock_pins);
/* JR-022412 */
	free(pb_graph_node->reset_pins);

	free(pb_graph_node->num_input_pins);
	free(pb_graph_node->num_output_pins);
	free(pb_graph_node->num_clock_pins);
/* JR-022412 */
	free(pb_graph_node->num_reset_pins);
...
/* JR-022412 */
	for(i = 0; i < pb->num_reset_ports; i++) {
		if(0 == strcmp(port_name, pb->reset_pins[i][0].port->name)) {
			if(pin < pb->reset_pins[i][0].port->num_pins) {
				return &pb->reset_pins[i][pin];
			} else {
				return NULL;
			}
		}
...
/* JR-022412 */
	print_tabs(fp, level);
	fprintf(fp, "Reset Ports: total ports %d\n", pb_graph_node->num_reset_ports);
	echo_pb_pins(pb_graph_node->reset_pins, pb_graph_node->num_reset_ports, level, fp);
