...
/* JR-022412	int in_port_index, out_port_index, clock_port_index; */
	int in_port_index, out_port_index, clock_port_index, reset_port_index;
...
/* JR-022412			if(pb_type->ports[i].type == IN_PORT && pb_type->ports[i].is_clock == FALSE) { 	*/
		...
		for(i = 0; i < pb_type->num_ports; i++) {
/* JR-022412			if(pb_type->ports[i].type == IN_PORT && pb_type->ports[i].is_clock == TRUE) { 	*/
...
/* JR-022412 */
		for(i = 0; i < pb_type->num_ports; i++) {
			if(pb_type->ports[i].type == IN_PORT && pb_type->ports[i].is_reset) {
				assert(logical_block[iblk].reset_net != OPEN);
				node_index = pb->pb_graph_node->reset_pins[reset_port_index][0].pin_count_in_cluster;
				fprintf(fpout, "clb_%d_rr_node_%d 2", clb_index, find_fanin_rr_node(pb, pb_type->ports[i].type, node_index));
				reset_port_index++;
			}
		}
...
static void print_pb(FILE *fpout, t_pb * pb, int clb_index) {
	...
	if(pb_type->num_modes == 0) {
		...
		out_port_index = 0;
		for(i = 0; i < pb_type->num_ports; i++) {
/* JR-022412			if(!pb_type->ports[i].is_clock) { 	*/
			if(!pb_type->ports[i].is_clock && !pb_type->ports[i].is_reset) {
				for(j = 0; j < pb_type->ports[i].num_pins; j++) {
