...
/* JR-022412 */
		port_index = 0;			
		for(i = 0; i < pb_type->num_ports; i++) {
			if(pb_type->ports[i].is_reset && pb_type->ports[i].type == IN_PORT) {
				print_tabs(fpout, tab_depth);
				fprintf(fpout, "\t\t<port name=\"%s\">", pb_graph_node->pb_type->ports[i].name);
				for(j = 0; j < pb_type->ports[i].num_pins; j++) {
					node_index = pb_graph_node->reset_pins[port_index][j].pin_count_in_cluster;
					print_interconnect(node_index, &column, tab_depth + 2, fpout);
				}
				fprintf(fpout, "</port>\n");
				port_index++;
...
/* JR-022412 */
	...
	for(i = 0; i < pb_type->num_ports; i++) {
		if(pb_type->ports[i].is_reset && pb_type->ports[i].type == IN_PORT) {
			...
			fprintf(fpout, "\t\t<port name=\"%s\">", pb_graph_node->pb_type->ports[i].name);
			for(j = 0; j < pb_type->ports[i].num_pins; j++) {
				node_index = pb->pb_graph_node->reset_pins[port_index][j].pin_count_in_cluster;
				if(pb_type->parent_mode == NULL) {
					print_net_name(rr_node[node_index].net_num, &column, tab_depth, fpout);
				} else {
					print_interconnect(node_index, &column, tab_depth + 2, fpout);
 ...
 * the cluster, in essentially a graph based format.                           */
    ...
    if(global_clocks)
	{
		...
		{
/* JR-022412		    if(is_clock[netnum]) 	*/
		    if(is_clock[netnum] || is_reset[netnum])
			{
