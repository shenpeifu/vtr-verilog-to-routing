...
/* JR-022412				Type->is_global_pin[pin_count] = Type->pb_type->ports[j].is_clock;	*/
				Type->is_global_pin[pin_count] = Type->pb_type->ports[j].is_clock ||
				                                 Type->pb_type->ports[j].is_reset;
				pin_count++;
...
/* JR-022412 */
			...
			Cur = FindFirstElement(Parent, "reset", FALSE);
		}
		while (Cur != NULL)
		{
...
/* JR-022412	pb_type->num_clock_pins = pb_type->num_input_pins = pb_type->num_output_pins = 0; 	*/
	pb_type->num_input_pins = pb_type->num_output_pins = pb_type->num_clock_pins = pb_type->num_reset_pins = 0;
	for(i = 0; i < pb_type->num_ports; i++) {
/* JR-022412		if(pb_type->ports[i].type == IN_PORT && pb_type->ports[i].is_clock == FALSE) { 	*/
...
/* 			} 										*/
		if(pb_type->ports[i].type == IN_PORT && !pb_type->ports[i].is_clock && !pb_type->ports[i].is_reset) {
			pb_type->num_input_pins += pb_type->ports[i].num_pins;
		} else if(pb_type->ports[i].type == IN_PORT && pb_type->ports[i].is_clock) {
			pb_type->num_clock_pins += pb_type->ports[i].num_pins;
		} else if(pb_type->ports[i].type == IN_PORT && pb_type->ports[i].is_reset) {
			pb_type->num_reset_pins += pb_type->ports[i].num_pins;
		} else if(pb_type->ports[i].type == OUT_PORT) {
			pb_type->num_output_pins += pb_type->ports[i].num_pins;
		}
	}
...
/* JR-022412 */
		port->is_reset = FALSE;
	} else if (0 == strcmp(Parent->name, "reset")) {
		port->type = IN_PORT;
		port->is_clock = FALSE;
/* JR-022412 */
		port->is_reset = TRUE;
...
/* JR-022412 */
				Prop = FindProperty(p, "is_reset", FALSE);
				if(Prop && my_atoi(Prop) != 0) {
					tp->is_reset = TRUE;
				}
				ezxml_set_attr(p, "is_clock", NULL);
/* JR-022412 */
				ezxml_set_attr(p, "is_reset", NULL);
				temp->inputs = tp;
				junk = p;
				p = ezxml_next(p);
				FreeNode(junk);
...
/* JR-022412 */
	copy->num_reset_pins = pb_type->num_reset_pins;
	copy->num_pb = 1;
...
/* JR-022412 */
		copy->ports[i].is_reset = pb_type->ports[i].is_reset;
...
/* JR-022412 */
		copy->annotations[i].reset = my_strdup(pb_type->annotations[i].reset);
...
/* JR-022412 */
		if(lut_pb_type->annotations[i].reset) {
			free(lut_pb_type->annotations[i].reset);
		}
	}
...
/* JR-022412		Type->num_pins = Type->capacity * (Type->pb_type->num_input_pins + Type->pb_type->num_output_pins + Type->pb_type->num_clock_pins); 	*/
		Type->num_pins = Type->capacity * (Type->pb_type->num_input_pins + Type->pb_type->num_output_pins + Type->pb_type->num_clock_pins + Type->pb_type->num_reset_pins);
...
/* JR-022412 	model_library = my_calloc(4, sizeof(t_model)); */
	model_library = my_calloc(5, sizeof(t_model));
...
/* JR-022412 */
	model_library[0].outputs->is_reset = FALSE;
...
/* JR-022412 */
	model_library[1].inputs->is_reset = FALSE;
	model_library[1].instances = NULL;
	model_library[1].next = &model_library[2];
	model_library[1].outputs = NULL;

	model_library[2].name = my_strdup("names");
	model_library[2].index = 3;
	model_library[2].inputs = my_calloc(1, sizeof(t_model_ports));
	model_library[2].inputs->dir = IN_PORT;
	model_library[2].inputs->name = my_strdup("in");
	model_library[2].inputs->next = NULL;
	model_library[2].inputs->size = 1;
	model_library[2].inputs->min_size = 1;
	model_library[2].inputs->index = 0;
	model_library[2].inputs->is_clock = FALSE;
/* JR-022412 */
	model_library[2].inputs->is_reset = FALSE;
...
/* JR-022412 */
	...
	model_library[2].outputs->name = my_strdup("out");
...
/* JR-022412 */
	model_library[2].outputs->is_reset = FALSE;

	model_library[3].name = my_strdup("latch");
 	model_library[3].index = 2;
	model_library[3].inputs = my_calloc(2, sizeof(t_model_ports));
	model_library[3].inputs[0].dir = IN_PORT;
	model_library[3].inputs[0].name = my_strdup("D");
...
/* JR-022412 */
	model_library[3].inputs[0].is_reset = FALSE;	
	model_library[3].inputs[1].dir = IN_PORT;
	model_library[3].inputs[1].name = my_strdup("clk");
	model_library[3].inputs[1].next = NULL;
...
/* JR-022412 */
	model_library[3].inputs[1].is_reset = FALSE;
	...
	model_library[3].outputs->name = my_strdup("Q");
...
/* JR-022412 */
	model_library[3].outputs->is_reset = FALSE;

/* JR-022412 */
	model_library[4].name = my_strdup("latch_reset");
	model_library[4].index = 3;
	model_library[4].inputs = my_calloc(3, sizeof(t_model_ports));
	model_library[4].inputs[0].dir = IN_PORT;
	model_library[4].inputs[0].name = my_strdup("D");
	model_library[4].inputs[0].next = &model_library[4].inputs[1];
	...
	model_library[4].inputs[0].is_reset = FALSE;	
	model_library[4].inputs[1].dir = IN_PORT;
	model_library[4].inputs[1].name = my_strdup("clk");
	model_library[4].inputs[1].next = &model_library[4].inputs[2];
	model_library[4].inputs[1].size = 1;
	model_library[4].inputs[1].min_size = 1;
	model_library[4].inputs[1].index = 0;
	model_library[4].inputs[1].is_clock = TRUE;	
	model_library[4].inputs[1].is_reset = FALSE;
	model_library[4].inputs[2].dir = IN_PORT;
	model_library[4].inputs[2].name = my_strdup("rst");
	model_library[4].inputs[2].next = NULL;
	model_library[4].inputs[2].size = 1;
	model_library[4].inputs[2].min_size = 1;
	model_library[4].inputs[2].index = 0;
	model_library[4].inputs[2].is_clock = FALSE;
	model_library[4].inputs[2].is_reset = TRUE;	
	model_library[4].instances = NULL;
	model_library[4].next = NULL;
	model_library[4].outputs = my_calloc(1, sizeof(t_model_ports));
	model_library[4].outputs->dir = OUT_PORT;
	model_library[4].outputs->name = my_strdup("Q");
	model_library[4].outputs->next = NULL;
	model_library[4].outputs->size = 1;
	model_library[4].outputs->min_size = 1;
	model_library[4].outputs->index = 0;
	model_library[4].outputs->is_clock = FALSE;
	model_library[4].outputs->is_reset = FALSE;
...
/* JR-022412 */
			(strcmp(blif_model_name, "latch_reset") == 0) ) {
