 ...
 for (bnum = 0; bnum < num_blocks; bnum++) {
	 if (block[bnum].is_LUT_buffer == TRUE) {
		 ...
		 if (net[out_net].num_pins == 2) {
			 ...
			 block[bnum].type = EMPTY;
		 }
		 else {
			 block[bnum].is_LUT_buffer = FALSE;
		 }
	 }
 }
...
/* Fold the LATCH into previous LUT block.  Update block and net data. */
			 ...
			 block[in_blk].lut_name = my_strdup(tmp_name);
 ...
 * have to be renumbered somewhat.                              */
 ...
 new_num_nets = 0;
 new_num_blocks = 0;
 net_remap = (int*)my_malloc (num_nets * sizeof(int));
 block_remap = (int*)my_malloc(num_blocks * sizeof(int));
