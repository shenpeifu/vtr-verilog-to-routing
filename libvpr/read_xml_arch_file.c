...
/* JR-041212 	num_ports = 0; */
/*  		num_ports += CountChildren(Parent, "input", 0); */
/*  		num_ports += CountChildren(Parent, "output", 0); */
/*  		num_ports += CountChildren(Parent, "clock", 0); */

	pb_type->num_input_ports = CountChildren(Parent, "input", 0);
	pb_type->num_output_ports = CountChildren(Parent, "output", 0);
	pb_type->num_clock_ports = CountChildren(Parent, "clock", 0);

	num_ports = pb_type->num_input_ports + 
	            pb_type->num_output_ports + 
	            pb_type->num_clock_ports;
...
/* JR-041212 */
			if(pb_type->ports[j].type == IN_PORT && pb_type->ports[j].is_clock == FALSE) {
				pb_type->equivalent_input_pins = pb_type->ports[j].equivalent;
			}
...
/* JR-041212  	pb_type->num_clock_pins = pb_type->num_input_pins = pb_type->num_output_pins = 0; */
	pb_type->num_input_pins = pb_type->num_output_pins = pb_type->num_clock_pins = pb_type->num_direct_pins = 0;
...
/* JR-041212 */
	/* Optionally increment input pin count when "num_interconnect_pins" defined */
	if(arch->num_interconnect_pins > 0) {

		/* Only increment input pin count if CLB is interconnect direct candidate */
		if(pb_type->num_output_ports == 1 && pb_type->num_input_ports == 1 && 
		   pb_type->num_output_pins >= 4 * arch->num_interconnect_pins &&
		   pb_type->equivalent_input_pins) {

			for(i = 0; i < pb_type->num_ports; i++) {
				if(pb_type->ports[i].type == IN_PORT && pb_type->ports[i].is_clock == FALSE) {
					pb_type->num_direct_pins += 4 * arch->num_interconnect_pins;
				}
			}
		}
	}
...
/* JR-041212 			ProcessMode(Parent, &pb_type->modes[i]); */
			ProcessMode(Parent, arch, &pb_type->modes[i]);
			i++;
		} else {
			pb_type->modes = my_calloc(pb_type->num_modes, sizeof(t_mode));
			...
			while (Cur != NULL)
			{
				if(0 == strcmp(Cur->name, "mode")) {
					pb_type->modes[i].parent_pb_type = pb_type;
/* JR-041212					ProcessMode(Cur, &pb_type->modes[i]); */
					ProcessMode(Cur, arch, &pb_type->modes[i]);
...
/* JR-041212 			ProcessPb_Type(Cur, &mode->pb_type_children[i], mode); */
			ProcessPb_Type(Cur, arch, &mode->pb_type_children[i], mode);
...
/* JR-041212 */
	    arch->C_ipin_direct = GetFloatProperty(Cur, "C_ipin_direct", FALSE, 0);
	    arch->T_ipin_direct = GetFloatProperty(Cur, "T_ipin_direct", FALSE, 0);
	    FreeNode(Cur);
	}

/* JR-041212 */
    Cur = FindElement(Node, "nearest_neighbor", FALSE);
    if(Cur != NULL)
	{
	    arch->num_interconnect_pins = GetIntProperty(Cur, "interconnect_direct", FALSE, 0);
	    FreeNode(Cur);
	}
...
/* JR-041212 */
	copy->num_direct_pins = pb_type->num_direct_pins;
	copy->num_pb = 1;
 ...
 * when complete. */ 
static void
ProcessComplexBlocks(INOUTP ezxml_t Node, 
/* JR-041212 */
		     INP struct s_arch *arch,
		     OUTP t_type_descriptor ** Types,
		     OUTP int *NumTypes, 
...
/* JR-041212 		ProcessPb_Type(CurType, Type->pb_type, NULL); */
		ProcessPb_Type(CurType, arch, Type->pb_type, NULL);
...
/* JR-041212 */
		Type->num_interconnects = Type->capacity * Type->pb_type->num_direct_pins;
...
/* JR-041212     ProcessComplexBlocks(Next, Types, NumTypes, timing_enabled); */
    ProcessComplexBlocks(Next, arch, Types, NumTypes, timing_enabled);
    FreeNode(Next);
...
/* JR-041212 */
	    fprintf(Echo, "\tnum_interconnects: %d\n", Types[i].num_interconnects);
